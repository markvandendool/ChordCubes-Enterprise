...
// DisposalManager - Critical Memory Leak Fix
class DisposalManager {
    constructor() {
        this.disposables = new Set();
        this.eventListeners = new Map();
        this.timers = new Set();
        console.log('[DISPOSAL MANAGER] 🛡️ Emergency memory management initialized');
    }
    ...
    disposeAll() {
        console.log('[DISPOSAL MANAGER] 🧹 Starting emergency cleanup...');
        ...
        for (const obj of this.disposables) {
            try {
                ...
                console.warn('[DISPOSAL MANAGER] ⚠️ Error disposing object:', error);
            }
        }
        ...
        for (const [key, listener] of this.eventListeners) {
            try {
                ...
                console.warn('[DISPOSAL MANAGER] ⚠️ Error removing event listener:', error);
            }
        }
        ...
        for (const timerId of this.timers) {
            try {
                ...
                console.warn('[DISPOSAL MANAGER] ⚠️ Error clearing timer:', error);
            }
        }
        ...
        console.log('[DISPOSAL MANAGER] ✅ Emergency cleanup completed');
    }
}

// Global disposal manager
const disposalManager = new DisposalManager();
window.disposalManager = disposalManager;

// Safe access utility for null pointer protection
function safeAccess(obj, path, defaultValue = null) {
    try {
        ...
        console.warn('[SAFE ACCESS] ⚠️ Access error:', error);
        return defaultValue;
    }
}
...
window.addEventListener('beforeunload', () => {
    console.log('[EMERGENCY] 🚨 Page unload - triggering emergency cleanup');
    disposalManager.disposeAll();
});

// ============================================
// 🚨 EMERGENCY AUDIO ENGINE REPLACEMENT
// ============================================

// Emergency AudioEngine replacement to fix missing OrchestralAudioEngine
class EmergencyAudioEngine {
    constructor() {
        ...
        console.log('[EMERGENCY AUDIO] 🚨 Emergency audio engine created as OrchestralAudioEngine replacement');
    }
    ...
    async init() {
        ...
        if (this.isInitializing) {
            console.log('[EMERGENCY AUDIO] ⏳ Already initializing, please wait...');
            return false;
        }
        ...
        try {
            console.log('[EMERGENCY AUDIO] 🔧 Initializing emergency audio engine...');

            // Ensure Tone.js is ready
            if (typeof Tone !== 'undefined' && Tone.context.state !== 'running') {
                await Tone.start();
                console.log('[EMERGENCY AUDIO] ✅ Tone.js context started');
            }

            // Create basic instruments
            this.createBasicInstruments();

            this.state.status = 'initialized';
            this.isInitializing = false;

            console.log('[EMERGENCY AUDIO] ✅ Emergency audio engine initialized successfully');
            return true;
            ...
            this.isInitializing = false;
            console.error('[EMERGENCY AUDIO] ❌ Initialization failed:', error);
            return false;
        }
    }
    ...
    createBasicInstruments() {
        try {
            if (typeof Tone !== 'undefined') {
                // Create basic synth instruments
                this.instruments.set('piano', new Tone.Synth().toDestination());
                this.instruments.set('bass', new Tone.FMSynth().toDestination());
                this.instruments.set('melody', new Tone.AMSynth().toDestination());
                console.log('[EMERGENCY AUDIO] 🎹 Basic instruments created');
            }
        } catch (error) {
            console.warn('[EMERGENCY AUDIO] ⚠️ Could not create basic instruments:', error);
        }
    }

    // Stub methods to prevent crashes
    playBass(note, duration, velocity) {
        try {
            const instrument = this.instruments.get('bass');
            if (instrument && typeof note === 'string') {
                instrument.triggerAttackRelease(note, duration || 0.5, Tone.now(), velocity || 0.5);
            }
        } catch (error) {
            console.warn('[EMERGENCY AUDIO] ⚠️ Bass playback failed:', error);
        }
    }

    playMelody(notes, durations, velocity) {
        try {
            const instrument = this.instruments.get('melody');
            if (instrument && Array.isArray(notes)) {
                let when = Tone.now();
                notes.forEach((note, i) => {
                    const duration = durations?.[i] || 0.5;
                    instrument.triggerAttackRelease(note, duration, when, velocity || 0.5);
                    when += duration;
                });
            }
        } catch (error) {
            console.warn('[EMERGENCY AUDIO] ⚠️ Melody playback failed:', error);
        }
    }

    refreshInstrumentDropdowns() {
        console.log('[EMERGENCY AUDIO] 📋 Instrument dropdown refresh requested');
        // Stub for compatibility
    }

    dispose() {
        try {
            for (const [name, instrument] of this.instruments) {
                if (instrument.dispose) {
                    instrument.dispose();
                }
            }
            this.instruments.clear();
            console.log('[EMERGENCY AUDIO] 🧹 Emergency audio engine disposed');
        } catch (error) {
            console.warn('[EMERGENCY AUDIO] ⚠️ Disposal error:', error);
        }
    }
}

// Register the emergency audio engine as OrchestralAudioEngine
window.OrchestralAudioEngine = EmergencyAudioEngine;
console.log('[EMERGENCY AUDIO] 🚨 OrchestralAudioEngine replaced with emergency implementation');
...
// 🚨 DEBUG: Test if noteSetsC is available and the function can be called
console.log('🚨 [STARTUP DEBUG] noteSetsC available:', !!noteSetsC);
console.log('🚨 [STARTUP DEBUG] noteSetsC.I:', noteSetsC?.I);
console.log('🚨 [STARTUP DEBUG] noteSetsC.ii:', noteSetsC?.ii);

// 🚨 DEBUG: Test function exists
console.log('🚨 [STARTUP DEBUG] updateChordFaceWithDiatonic7th function exists:', typeof updateChordFaceWithDiatonic7th);
...
// Check if we're in performance mode (bass/melody locked)
function isPerformanceMode() {
    return lineup.length && lockedMelody && lockedBass &&
        lockedMelody.length === lineup.length && lockedBass.length === lineup.length;
}

// Apply appropriate lighting based on performance mode
function applyPerformanceLighting() {
    if (isPerformanceMode()) {
        // Performance mode: Keep shelf dark, focus on front row
        ...
        // Front row should remain lit for readability
    }
}
...
// 🚨 EMERGENCY FIX: Register geometry with disposal manager
disposalManager.register(geometry);
console.log('[MEMORY LEAK FIX] ✅ Shared cube geometry registered for disposal');
...
function makeFrontLabelTextureStyled(labelText, romanLabel) {
    // Reduced debug logging for production - V1.62
    if (window.DEBUG_FONTS) console.log(`🔧 [DEBUG] makeFrontLabelTextureStyled called: "${labelText}" (${romanLabel})`);
    const size = 1024;
    ...
    try {
        ...
        if (document.fonts && document.fonts.check) {
            fontReady = document.fonts.check('100px Fontdec13Upgraded');
            if (fontReady) {
                if (window.DEBUG_FONTS) console.log(`[FONT V1.62] ✅ Fontdec13Upgraded ready via document.fonts API for ${labelText}`);
            }
        }

        // METHOD 2: Fallback canvas measurement (legacy support)
        if (!fontReady) {
            ctx.font = "400 100px Fontdec13Upgraded";
            const testWidth1 = ctx.measureText("I").width;
            ctx.font = "400 100px serif";
            const testWidth2 = ctx.measureText("I").width;

            // If widths are different, Fontdec13Upgraded is loaded
            fontReady = Math.abs(testWidth1 - testWidth2) > 1;

            if (fontReady) {
                if (window.DEBUG_FONTS) console.log(`[FONT V1.62] ✅ Fontdec13Upgraded ready via canvas measurement for ${labelText}`);
            } else {
                if (window.DEBUG_FONTS) console.warn(`[FONT V1.62] ⚠️ Fontdec13Upgraded not detected - using fallback for ${labelText}`);
            }
        }

        // METHOD 3: Force fallback if still not ready
        if (!fontReady) {
            if (window.DEBUG_FONTS) console.warn(`[FONT V1.62] 🔄 Using serif fallback for ${labelText} - font may still be loading`);
        }

    } catch (e) {
        if (window.DEBUG_FONTS) console.error(`[FONT V1.62] ❌ Font detection failed for ${labelText}:`, e);
        fontReady = false;
    }
    ...
    // STAGE 1: MAXIMUM SIZE FIRST APPROACH (User Priority: "MAXIMUM size")
    // CRITICAL: Always start with maximum size - only shrink if measurements prove overflow
    const MAXIMUM_SIZE = 460;  // User requirement: MAXIMUM size priority
    const MINIMUM_SIZE = 180;  // Minimum readable threshold  
    let baseSize = MAXIMUM_SIZE; // NO pre-shrinking based on length - measure first!

    // STAGE 2: PRECISE BOUNDARY CALCULATION
    // Professional typography requires exact measurement zones
    const safetyMargin = borderPx * 2.0; // RESTORED: Reasonable boundary for MAXIMUM size with smart shrinking
    const maxTextWidth = size - safetyMargin; // Tighter safe zone to trigger shrinking
    const maxTextHeight = size - safetyMargin;
    ...
    // PRE-CALCULATE ligature compensation for width fitting (Solution 1: Hybrid Width Calculation)
    let fittingCompensationRatio = 0;
    const hasSeventhSuffix = /[7ø]$/.test(basePretty);
    const hasAddedNumerals = /[+][A-G][b#]?/.test(basePretty);
    const hasFullDiminished = /[º]/.test(basePretty);      // Full diminished (much larger ligature)
    const hasHalfDiminished = /[ø]/.test(basePretty);      // Half diminished (smaller ligature)
    const hasDiminishedSymbols = /[ºø]/.test(basePretty);  // Any diminished (kept for compatibility)
    const hasSeventhInMiddle = /[ø][7]/.test(basePretty);
    const hasNumbers = /[IVX]+[0-9]/.test(basePretty);

    if (hasSeventhSuffix || hasAddedNumerals || hasDiminishedSymbols || hasSeventhInMiddle || hasNumbers) {
        // PRIORITY ORDER: Most specific patterns first
        if (hasSeventhInMiddle) {
            fittingCompensationRatio = 0.25;
        } else if (hasFullDiminished) {
            // º symbols need aggressive compensation - "made much much bigger by ligatures"
            fittingCompensationRatio = 0.25;
        } else if (hasHalfDiminished && hasSeventhSuffix) {
            // ø with 7th suffix (like #vø7) 
            fittingCompensationRatio = 0.15;
        } else if (hasAddedNumerals) {
            fittingCompensationRatio = 0.12;
        } else if (hasNumbers) {
            fittingCompensationRatio = 0.15;
        } else if (hasHalfDiminished) {
            // ø symbols alone
            fittingCompensationRatio = 0.12;
        } else if (hasSeventhSuffix) {
            fittingCompensationRatio = 0.08;
        }
    }

    // Include ligature compensation in fitting width calculations
    const fittingCompensation = baseSize * fittingCompensationRatio;
    let actualWidth = textMetrics.width + fittingCompensation;
    let actualHeight = baseSize * 1.15;

    console.log(`🔧 [DEBUG] Text analysis: "${basePretty}" - baseSize=${baseSize}px, textWidth=${textMetrics.width.toFixed(1)}px, compensation=${fittingCompensation.toFixed(1)}px, totalWidth=${actualWidth.toFixed(1)}px`);
    if (fittingCompensationRatio > 0) {
        console.log(`[HYBRID WIDTH] ${basePretty}: Fitting compensation ${fittingCompensationRatio * 100}% (${fittingCompensation.toFixed(1)}px) included in width calculations`);
    }

    // STAGE 4: INTELLIGENT TEXT FITTING ALGORITHM
    // Industry standard: Try horizontal adjustment BEFORE font size reduction
    let horizontalOffset = 0; // Start centered
    let finalFontSize = baseSize;

    // PRIORITY 1: Horizontal positioning optimization
    if (actualWidth > optimalTextWidth && actualWidth <= maxTextWidth) {
        // Text fits but needs repositioning for perfect centering
        const overflow = actualWidth - optimalTextWidth;
        horizontalOffset = -overflow / 4; // Subtle left shift to maintain balance
        console.log(`[TEXT POSITIONING] ${basePretty}: Shifted ${horizontalOffset.toFixed(1)}px left for optimal centering`);
    }

    // PRIORITY 2: Font size reduction (ONLY when absolutely necessary)
    console.log(`🔧 [DEBUG] Width check: actualWidth=${actualWidth.toFixed(1)}, maxTextWidth=${maxTextWidth.toFixed(1)}, overflow=${actualWidth > maxTextWidth}`);
    if (actualWidth > maxTextWidth || actualHeight > maxTextHeight) {
        console.log(`[FONT SCALING] ${basePretty}: Text overflow detected, applying industry-standard scaling`);

        // SOLUTION 1: MAXIMUM FIRST + ITERATIVE PRECISION SHRINKING
        let currentSize = MAXIMUM_SIZE;
        const SHRINK_STEP = 5; // Small steps for precision
        let iterations = 0;
        const maxIterations = Math.floor((MAXIMUM_SIZE - MINIMUM_SIZE) / SHRINK_STEP);

        while (currentSize >= MINIMUM_SIZE && iterations < maxIterations) {
            ctx.font = `400 ${currentSize}px ${SERIF_STACK}`;
            const testMetrics = ctx.measureText(basePretty);

            // Calculate total width including ligature compensation
            const testCompensation = currentSize * fittingCompensationRatio;
            const totalTestWidth = testMetrics.width + testCompensation;
            const testHeight = currentSize * 1.15;

            // 🎯 CRITICAL: Also test positioning offset to prevent edge overflow
            let testPositioningOffset = 0;
            if ('actualBoundingBoxLeft' in testMetrics && 'actualBoundingBoxRight' in testMetrics) {
                const leftExtent = testMetrics.actualBoundingBoxLeft;
                const rightExtent = testMetrics.actualBoundingBoxRight;
                const visualCenterOffset = (rightExtent - leftExtent) / 2;
                testPositioningOffset = Math.abs(visualCenterOffset); // Use absolute value for edge checking
            }

            // Calculate effective width including positioning offset
            const effectiveWidth = totalTestWidth + testPositioningOffset;

            // Check if this size fits with 95% safety margin (user wants readable, not cramped)
            if (effectiveWidth <= maxTextWidth * 0.95 && testHeight <= maxTextHeight * 0.95) {
                console.log(`🎯 [OPTIMAL SIZE FOUND] ${basePretty}: ${currentSize}px (${iterations} iterations) - effectiveWidth=${effectiveWidth.toFixed(1)}`);
                finalFontSize = currentSize;
                textMetrics = testMetrics;
                actualWidth = totalTestWidth;
            ...
            iterations++;
            console.log(`[PRECISION SHRINK ${iterations}] ${basePretty}: Testing ${currentSize}px - effectiveWidth: ${effectiveWidth.toFixed(1)}/${(maxTextWidth * 0.95).toFixed(1)} (text=${totalTestWidth.toFixed(1)} + pos=${testPositioningOffset.toFixed(1)})`);
        }

        if (currentSize < MINIMUM_SIZE) {
            console.warn(`⚠️ [MINIMUM REACHED] ${basePretty}: Using minimum size ${MINIMUM_SIZE}px`);
            finalFontSize = MINIMUM_SIZE;
            ctx.font = `400 ${finalFontSize}px ${SERIF_STACK}`;
            textMetrics = ctx.measureText(basePretty);
            actualWidth = textMetrics.width + (finalFontSize * fittingCompensationRatio);
            actualHeight = finalFontSize * 1.15;
        }

        // Reset horizontal offset after scaling (text may now fit optimally)
        if (actualWidth <= optimalTextWidth) {
            horizontalOffset = 0; // Perfect center after scaling
        }
    } else {
        console.log(`✅ [MAXIMUM SIZE PERFECT] ${basePretty}: Fits perfectly at ${finalFontSize}px - no shrinking needed!`);
    }

    // STAGE 5: FINAL QUALITY ASSURANCE
    // Ensure absolute text containment (industry safety requirement)
    // Recalculate actual width with final font size compensation
    ctx.font = `400 ${finalFontSize}px ${SERIF_STACK}`;
    const finalTextMetrics = ctx.measureText(basePretty);
    const finalFittingCompensation = finalFontSize * fittingCompensationRatio;
    const finalActualWidth = finalTextMetrics.width + finalFittingCompensation;

    if (finalActualWidth > maxTextWidth) {
        finalFontSize = Math.floor(finalFontSize * (maxTextWidth / finalActualWidth) * 0.95);
        horizontalOffset = 0; // Reset to center for emergency sizing
        console.log(`[QUALITY ASSURANCE] ${basePretty}: Emergency fit at ${finalFontSize}px (was ${finalActualWidth.toFixed(1)}px, max: ${maxTextWidth.toFixed(1)}px)`);
    }
    ...
    // ================================
    // 🔧 FONT JAN16 LIGATURE COMPENSATION SYSTEM
    // ================================
    // Font Jan16 uses zero/negative width ligature characters for 7ths, ø, b, etc.
    // This breaks canvas centering calculations, requiring mathematical compensation

    let ligatureCompensation = 0;

    // Reuse pattern detection from fitting stage (variables already declared above)

    if (hasSeventhSuffix || hasAddedNumerals || hasDiminishedSymbols || hasSeventhInMiddle || hasNumbers) {
        // Calculate proportional compensation based on font size
        // PRIORITY ORDER: Most specific patterns first (matching fitting stage)
        let compensationRatio = 0;

        if (hasSeventhInMiddle) {
            // Most severe case: ø7 ligature completely off-screen
            compensationRatio = 0.25; // Move left by 25% of font size
            console.log(`[LIGATURE FIX] ø7 pattern detected in "${basePretty}" - applying 25% compensation`);
        } else if (hasFullDiminished) {
            // º symbols need aggressive compensation - "made much much bigger by ligatures"
            compensationRatio = 0.25; // Move left by 25% of font size
            console.log(`[LIGATURE FIX] Full diminished º pattern detected in "${basePretty}" - applying 25% compensation`);
        } else if (hasHalfDiminished && hasSeventhSuffix) {
            // ø with 7th suffix (like #vø7) 
            compensationRatio = 0.15; // Move left by 15% of font size  
            console.log(`[LIGATURE FIX] Half diminished + 7th pattern detected in "${basePretty}" - applying 15% compensation`);
        } else if (hasAddedNumerals) {
            // Added note patterns like IV+E
            compensationRatio = 0.12; // Move left by 12% of font size
            console.log(`[LIGATURE FIX] Added numeral pattern detected in "${basePretty}" - applying 12% compensation`);
        } else if (hasNumbers) {
            // Roman+number patterns like IV2 → IV(2)
            compensationRatio = 0.15; // Move left by 15% of font size
            console.log(`[LIGATURE FIX] Roman+number pattern detected in "${basePretty}" - applying 15% compensation`);
        } else if (hasHalfDiminished) {
            // ø symbols alone
            compensationRatio = 0.12; // Move left by 12% of font size
            console.log(`[LIGATURE FIX] Half diminished ø pattern detected in "${basePretty}" - applying 12% compensation`);
        } else if (hasSeventhSuffix) {
            // Basic case: simple 7th endings
            compensationRatio = 0.08; // Move left by 8% of font size
            console.log(`[LIGATURE FIX] 7th suffix detected in "${basePretty}" - applying 8% compensation`);
        }

        // Apply proportional compensation (negative = move left to re-center)
        ligatureCompensation = -(baseSize * compensationRatio);
        console.log(`[LIGATURE FIX] Font size: ${baseSize}px, Compensation: ${ligatureCompensation.toFixed(1)}px`);
    }

    // PROFESSIONAL TEXT RENDERING - Industry Standard Centering with Ligature Fix
    ctx.save();
    ctx.fillStyle = fill;
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.font = `400 ${baseSize}px ${cochin}`;

    // 🎯 PRECISION POSITIONING: Detect actual rendered text bounds
    // Use TextMetrics to find where the text will actually appear
    const preciseMetrics = ctx.measureText(basePretty);

    // Use advanced TextMetrics if available (modern browsers)
    let perfectCenteringOffset = 0;

    if ('actualBoundingBoxLeft' in preciseMetrics && 'actualBoundingBoxRight' in preciseMetrics) {
        // TextMetrics with actualBoundingBox gives us precise visual bounds
        const leftExtent = preciseMetrics.actualBoundingBoxLeft;   // positive = extends left of center
        const rightExtent = preciseMetrics.actualBoundingBoxRight; // positive = extends right of center

        // Calculate the visual center offset from the text alignment point (center)
        const visualCenterOffset = (rightExtent - leftExtent) / 2;

        // To achieve perfect visual centering, move by the negative of this offset
        perfectCenteringOffset = -visualCenterOffset;

        console.log(`[PRECISE BOUNDS] ${basePretty}: left=${leftExtent.toFixed(1)}, right=${rightExtent.toFixed(1)}, visualCenterOffset=${visualCenterOffset.toFixed(1)}, correction=${perfectCenteringOffset.toFixed(1)}`);
    } else {
        // Fallback: Use traditional ligature compensation for older browsers
        console.log(`[FALLBACK BOUNDS] ${basePretty}: Using traditional ligature compensation`);
        perfectCenteringOffset = ligatureCompensation;
    }

    // Compare old vs new positioning methods for debugging
    const oldRenderX = centerX + horizontalOffset + ligatureCompensation;
    const newRenderX = centerX + perfectCenteringOffset;

    console.log(`[POSITIONING] ${basePretty}: OLD_X=${oldRenderX.toFixed(1)}, NEW_X=${newRenderX.toFixed(1)}, correction=${(newRenderX - oldRenderX).toFixed(1)}px`);

    // Apply precision positioning (use actual bounds instead of mathematical compensation)
    const renderX = newRenderX;
    ctx.fillText(basePretty, renderX, centerY);

    console.log(`[TYPOGRAPHY SUCCESS] ${basePretty}: Rendered at ${baseSize}px, position offset: ${horizontalOffset.toFixed(1)}px, render compensation: ${ligatureCompensation.toFixed(1)}px`);
    ctx.restore();

    // NO SUPERSCRIPTS - Font Jan16 ligature system handles 7th notation naturally

    // ENHANCED Applied-chord annotation with boundary respect - V1.61 Upgrade
    const annotation = annotationForRoman(romanLabel);
    if (annotation) {
        // Remove spaces around 'of' to enable ligature functionality in Fontdec13Upgraded font
        const a = toMusicalGlyphs(annotation).replace(/ of /g, 'of');

        // IMPROVED ANNOTATION STYLING - Match template requirements
        let annotationSize = 150; // Increased base size (was 130)
        // Use Fontdec13Upgraded font specifically for annotations to match cube face
        // Using the exact font name ensures the ligature features are applied properly
        ctx.font = `400 ${annotationSize}px 'Fontdec13Upgraded', ${SERIF_STACK}`;

        // Check if annotation fits within safe boundaries
        const bracketSpacing = 20; // Reduced from 40 to tighten spacing
        const annotationMetrics = ctx.measureText(a);
        const annotationWidth = annotationMetrics.width + bracketSpacing;
        const availableAnnotationWidth = size - (borderPx * 3); // Increased safety margin

        // Scale down annotation if it's too wide
        if (annotationWidth > availableAnnotationWidth) {
            annotationSize = Math.floor(annotationSize * (availableAnnotationWidth / annotationWidth) * 0.95);
            ctx.font = `400 ${annotationSize}px ${SERIF_STACK}`;
            console.log(`[ANNOTATION RESIZE] ${a}: scaled to ${annotationSize}px for boundary safety`);
        }

        ctx.textAlign = 'center';
        // Use top baseline for better vertical positioning
        ctx.textBaseline = 'middle';
        ctx.fillStyle = fill;

        // IMPROVED POSITIONING - Raise baseline as requested
        // Position annotation higher above bottom border to match template
        const annY = size - Math.max(borderPx + 85, 165); // Higher position (was borderPx + 30, 105)

        // RENDER ANNOTATION TEXT
        // With Fontdec13Upgraded font, we can rely on the font's ligature features
        // for proper spacing and rendering of the "of" ligature
        const annotationText = `[${a}]`;

        // Use the font's natural ligatures while maintaining good spacing
        ctx.fillText(annotationText, centerX, annY);
    }
...
function makeWoodPanel(ctx, size) {
    // LIGHTENED BY 40% FOR BETTER TEXT LEGIBILITY (Item #23)
    // Original: '#d5b38a' → Lightened: '#e8d4b8' 
    // Original: '#b88d5f' → Lightened: '#d4b48a'
    const grd = ctx.createLinearGradient(0, 0, 0, size);
    grd.addColorStop(0, '#e8d4b8'); // 40% lighter tan base
    grd.addColorStop(1, '#d4b48a'); // 40% lighter brown
    ctx.fillStyle = grd;
    ctx.fillRect(0, 0, size, size);
    // subtle grain lines (also lightened)
    ctx.strokeStyle = 'rgba(90,60,30,0.05)'; // Reduced from 0.08 to 0.05 for subtlety
    ctx.lineWidth = 3; // Reduced from 4 to 3 for less intrusion
...
// Format optimized for Fontdec13Upgraded font ligatures
...
async function refreshAllCubeFaces(previewWith7th = false) {
    console.log(`[LEGIBILITY UPDATE] ✨ Applying 40% lighter wood backgrounds and enhanced text boundaries (Items #19, #23)`);
    console.log(`[7TH FIX V1.59] Refreshing faces - previewWith7th: ${previewWith7th}, withSeventh: ${withSeventh}`);
    console.log(`🔥 [PIPELINE DEBUG] Front-row cubes count: ${cubes.length}`);

    // Enhanced debugging for 7th chord issues
    if (previewWith7th) {
        console.log(`[7TH FIX V1.59] 🎵 7TH PREVIEW MODE ACTIVATED - All eligible chords will show 7th notation`);
    } else {
        console.log(`[7TH FIX V1.59] 🎵 NORMAL MODE - Using global withSeventh setting: ${withSeventh}`);
    }
    ...
    try {
        ...
        for (const cube of cubes) {
            if (cube.userData && cube.userData.roman) {
                ...
                // ENHANCED 7TH CHORD LOGIC V1.61 - More permissive for shift+method
                const isDiminished = roman.includes('º') || roman.includes('ø');
                const isIb7 = roman === 'Ib7' || roman.includes('Ib7');

                // CRITICAL FIX: Only exclude chords that ALREADY have 7th notation
                // Don't exclude based on chord type, only on existing notation
                ...
                if (shouldExcludeFromPreview) {
                    // These chords already have 7th, don't add more
                    effectiveWith7th = withSeventh; // Use normal setting
                    console.log(`[7TH FIX V1.61] ${roman}: Excluded from 7th preview (already has 7th notation)`);
                } else {
                    // Apply 7th preview to ALL eligible chords (major improvement)
                    effectiveWith7th = previewWith7th || withSeventh;
                    if (previewWith7th) {
                        console.log(`[7TH FIX V1.61] ${roman}: ✅ APPLYING 7th preview - FORCE ENABLED (should show 7th notation)`);
                    } else {
                        console.log(`[7TH FIX V1.61] ${roman}: Normal mode, withSeventh=${withSeventh}`);
                    }
                }
...
// OLD FUNCTION - DEPRECATED: Was for CTRL+CLICK but now unused (SHIFT+CLICK uses updateChordFaceWithDiatonic7th)
// CTRL+CLICK (CMD+CLICK on Mac): Temporarily update front face with 7th notation
function updateChordFaceWith7th_DEPRECATED(targetObj) {
    try {
...
// Helper function to determine the type of 7th interval
function getSeventhType(rootNote, seventhNote) {
    // Convert notes to semitones for interval calculation
    const SEMITONE_FOR_NOTE = {
        'C': 0, 'C#': 1, 'Db': 1, 'D': 2, 'D#': 3, 'Eb': 3, 'E': 4, 'Fb': 4, 'E#': 5,
        'F': 5, 'F#': 6, 'Gb': 6, 'G': 7, 'G#': 8, 'Ab': 8, 'A': 9, 'A#': 10, 'Bb': 10,
        'B': 11, 'Cb': 11, 'B#': 0
    };

    const rootSemitone = SEMITONE_FOR_NOTE[rootNote] || 0;
    const seventhSemitone = SEMITONE_FOR_NOTE[seventhNote] || 0;
    const interval = (seventhSemitone - rootSemitone + 12) % 12;

    console.log(`[7TH TYPE DEBUG] ${rootNote} to ${seventhNote}: ${interval} semitones`);

    if (interval === 11) {
        return 'major7th'; // Major 7th (11 semitones)
    } else if (interval === 10) {
        return 'minor7th'; // Minor 7th (10 semitones)
    } else if (interval === 9) {
        return 'diminished7th'; // Diminished 7th (9 semitones)
    } else {
        console.warn(`[7TH TYPE] Unknown 7th interval: ${interval} semitones from ${rootNote} to ${seventhNote}`);
        return 'minor7th'; // Default to minor 7th
    }
}

// NEW FUNCTION for shift+click: Show correct diatonic 7th notation on cube face
function updateChordFaceWithDiatonic7th(targetObj) {
    console.log('🚨 [SHIFT+CLICK] *** FUNCTION CALLED *** updateChordFaceWithDiatonic7th()');
    console.log('🚨 [SHIFT+CLICK] targetObj:', targetObj);
    console.log('🚨 [SHIFT+CLICK] targetObj.userData:', targetObj?.userData);
    console.log('🚨 [SHIFT+CLICK] targetObj.material:', targetObj?.material);

    try {
        const roman = targetObj.userData.roman;

        console.log(`🚨 [SHIFT+CLICK DEBUG] Starting updateChordFaceWithDiatonic7th for ${roman}`);

        // Get the chord notes from noteSetsC
        const notesInC = noteSetsC[roman];
        if (!notesInC || notesInC.length < 4) {
            console.warn(`[SHIFT+CLICK] No 7th note found for chord ${roman}, notesInC:`, notesInC);
            return;
        }

        // Get the root and 7th notes in C major
        const rootInC = notesInC[0];
        const seventhInC = notesInC[3];

        // Transpose both notes to current key
        const transposedRoot = transposeNotes([rootInC], currentKey)[0];
        const transposedSeventh = transposeNotes([seventhInC], currentKey)[0];

        console.log(`[SHIFT+CLICK] ${roman} root: ${rootInC} → ${transposedRoot}, 7th: ${seventhInC} → ${transposedSeventh} (key: ${currentKey})`);

        // Determine the type of 7th interval
        const seventhType = getSeventhType(rootInC, seventhInC);

        // Create proper 7th notation based on type
        let seventhNotation;
        switch (seventhType) {
            case 'major7th':
                seventhNotation = 'Maj7'; // User requested Maj7 with capital M or & glyph
                break;
            case 'minor7th':
                seventhNotation = '7'; // User requested just "7" or "b7" - same result
                break;
            case 'diminished7th':
                seventhNotation = 'bb7'; // User requested bb7 for double flat 7th
                break;
            default:
                seventhNotation = '7';
                break;
        ...
        // Create label showing base chord + proper 7th notation
        const displayWith7th = `${roman}${seventhNotation}`;

        console.log(`🚨 [SHIFT+CLICK] ${roman} 7th type: ${seventhType} → notation: ${seventhNotation}`);
        console.log(`🚨 [SHIFT+CLICK] Updating face: ${roman} → ${displayWith7th}`);

        // Generate new texture with proper 7th notation  
        console.log(`🚨 [TEXTURE DEBUG] Calling makeFrontLabelTextureStyled('${displayWith7th}', '${roman}')`);
        const newTexture = makeFrontLabelTextureStyled(displayWith7th, roman);
        console.log(`🚨 [TEXTURE DEBUG] Generated texture:`, newTexture);

        // Update the front face (index 5)
        const frontFaceIndex = 5;
        console.log(`🚨 [MATERIAL DEBUG] targetObj.material:`, targetObj.material);
        console.log(`🚨 [MATERIAL DEBUG] Is material an array?:`, Array.isArray(targetObj.material));
        console.log(`🚨 [MATERIAL DEBUG] Material length:`, targetObj.material?.length);
        console.log(`🚨 [MATERIAL DEBUG] frontFaceIndex=${frontFaceIndex}, exists?:`, !!targetObj.material?.[frontFaceIndex]);
        if (targetObj.material && targetObj.material[frontFaceIndex]) {
            console.log(`🚨 [MATERIAL DEBUG] Material[${frontFaceIndex}] current map:`, targetObj.material[frontFaceIndex].map);
            targetObj.material[frontFaceIndex].map = newTexture;
            targetObj.material[frontFaceIndex].needsUpdate = true;
            console.log(`🚨 [SHIFT+CLICK] Updated front face (index ${frontFaceIndex}) with diatonic 7th notation: ${displayWith7th}`);
            console.log(`🚨 [MATERIAL DEBUG] Material[${frontFaceIndex}] new map:`, targetObj.material[frontFaceIndex].map);
        } else {
            console.error(`🚨 [MATERIAL ERROR] Cannot update face - material[${frontFaceIndex}] not found!`);
            console.error(`🚨 [MATERIAL ERROR] Available material indices:`, Object.keys(targetObj.material || {}));
        }

        // Force immediate renderer update for shift+click
        if (pipelineTracer && activeExtensions.size > 0) {
            console.log(`🔥 [PIPELINE] Stage 12 HARD-WIRED: Extension detected, forcing immediate refresh for SHIFT+CLICK ${roman}`);
            pipelineTracer.triggerStage(12, 'Hard-wired extension refresh (SHIFT+CLICK)', {
                cubeRoman: roman,
                cubeType: 'SHIFT+CLICK',
                extensionCount: activeExtensions.size,
                bypassDetection: true
            });
        ...
        // Reset back to original after 2 seconds
        setTimeout(() => {
            console.log(`[SHIFT+CLICK] Resetting face back to: ${roman}`);
            const originalTexture = loadFaceTexture(roman, roman);
            if (targetObj.material && targetObj.material[frontFaceIndex]) {
                targetObj.material[frontFaceIndex].map = originalTexture;
                targetObj.material[frontFaceIndex].needsUpdate = true;

                // Force immediate renderer update for reset
                if (pipelineTracer && activeExtensions.size > 0) {
                    console.log(`🔥 [PIPELINE] Stage 12 HARD-WIRED: Extension detected, forcing immediate refresh for SHIFT+CLICK RESET ${roman}`);
                    pipelineTracer.triggerStage(12, 'Hard-wired extension refresh (SHIFT+CLICK RESET)', {
                        cubeRoman: roman,
                        cubeType: 'SHIFT+CLICK RESET',
                        extensionCount: activeExtensions.size,
                        bypassDetection: true
                    });
        ...
        console.error('[SHIFT+CLICK] Error updating chord face with diatonic 7th:', error);
    }
}

// 🎵 NVX DIAMOND FONT LIGATURE SYSTEM V1.62 🎵

// NVX CHORD FUNCTION MAPPING: Scale degree → Circle color suffix
const NVX_CHORD_FUNCTION_MAP = {
    // ROOT positions (green circle)
    'root': 'r',

    // 3RD positions (red circle) 
    'major_3rd': '3rd',
    'minor_3rd': 'b3rd',

    // 5TH positions (blue circle)
    'perfect_5th': '5th',
    'diminished_5th': 'b5th',
    'augmented_5th': '#5th',

    // 7TH positions (gray circle)
    'major_7th': '7th',     // CORRECTED: Natural/Major 7th uses just '7th'
    'minor_7th': 'b7th',
    'diminished_7th': 'bb7th'
};

function determineChordFunction(scaleDegree, faceIndex, chordRoman) {
    /**
     * Determines chord function suffix based on scale degree, face position, and chord context
     * @param {string} scaleDegree - Scale degree (e.g., '1', 'b7', '#4')
     * @param {number} faceIndex - Face position (0=root, 1=3rd, 2=5th, 3=7th)
     * @param {string} chordRoman - Roman numeral (e.g., 'vi', 'ii', 'V7') for context
     * @returns {string} Chord function suffix for NVX ligature
     */

    // Face index mapping: [root, 3rd, 5th, 7th]
    switch (faceIndex) {
        case 0: // Root face (bottom)
            return 'r';

        case 1: // 3rd face (right)  
            // Check explicit scale degree markings first
            if (scaleDegree.includes('b3')) {
                return 'b3rd';
            }
            // Check chord context for ALL chords with minor 3rds
            if (chordRoman) {
                // Minor chords (lowercase): ii, iii, vi, i, v, iv
                if (chordRoman.includes('ii') || chordRoman.includes('iii') ||
                    chordRoman.includes('vi') || chordRoman.includes('i') || chordRoman.includes('v') ||
                    chordRoman.includes('iv')) {
                    return 'b3rd';
                }
                // Half-diminished chords (ø): viiø, iiiø, iiø, #ivø
                if (chordRoman.includes('ø')) {
                    return 'b3rd';
                }
                // Fully diminished chords (º): #vº, viiº7, #iiº, #iº  
                if (chordRoman.includes('º')) {
                    return 'b3rd';
                }
            }
            return '3rd';
        ...
        case 2: // 5th face (top)
            // Check explicit scale degree markings first
            if (scaleDegree.includes('b5') || scaleDegree.includes('b6')) {
                return 'b5th';
            } else if (scaleDegree.includes('#5') || scaleDegree.includes('#4')) {
                return '#5th';
            }
            // Check chord context for altered 5ths
            if (chordRoman) {
                // Half-diminished chords typically have b5th
                if (chordRoman.includes('ø')) {
                    return 'b5th';
                }
                // Some applied chords have #5th
                if (chordRoman.includes('III(7)')) {
                    return '#5th';
                }
            }
            return '5th';

        case 3: // 7th face (left)
            // Check explicit scale degree markings first
            if (scaleDegree.includes('bb7') || (scaleDegree.includes('b6') && chordRoman.includes('º'))) {
                return 'bb7th';
            } else if (scaleDegree.includes('b7')) {
                return 'b7th';
            }
            // Check chord context for 7th qualities
            if (chordRoman) {
                // Fully diminished chords (º): bb7th - #vº, viiº7, #iiº, #iº
                if (chordRoman.includes('º')) {
                    return 'bb7th';
                }
                // Half-diminished chords (ø): b7th - viiø, iiiø, iiø, #ivø  
                if (chordRoman.includes('ø')) {
                    return 'b7th';
                }
                // Minor chords and flat major chords: b7th
                if (chordRoman.includes('ii') || chordRoman.includes('iii') || chordRoman.includes('vi') ||
                    chordRoman.includes('i') || chordRoman.includes('v') || chordRoman.includes('iv') ||
                    chordRoman.includes('bVII') || chordRoman.includes('bVI') || chordRoman.includes('bIII')) {
                    return 'b7th';
                }
                // Specific 7th chords that are dominant (major with b7th): I7, V7, etc.
                if (chordRoman.includes('I7') || chordRoman.includes('V7') || chordRoman.includes('(7)')) {
                    return 'b7th';
                }
            }
            return '7th'; // Natural/Major 7th
...
function generateNVXLigature(scaleDegree, faceIndex, isLetterMode = false, currentKey = 'C', chordRoman = '') {
    /**
     * Generates NVX Diamond Font ligature text
     * @param {string} scaleDegree - Scale degree (e.g., '1', 'b7', '#4')
     * @param {number} faceIndex - Face position (0=root, 1=3rd, 2=5th, 3=7th)
     * @param {boolean} isLetterMode - Use letter names instead of scale degrees
     * @param {string} currentKey - Current musical key for transposition
     * @param {string} chordRoman - Roman numeral for chord context (e.g., 'vi', 'ii')
     * @returns {string} NVX ligature text (e.g., '1r', 'C r', 'b7b7th')
     */

    const chordFunctionSuffix = determineChordFunction(scaleDegree, faceIndex, chordRoman);

    if (isLetterMode) {
        // Convert scale degree to letter note with proper spacing
        // Use existing transposition logic if available
        const letterNote = scaleDegree; // TODO: Implement proper scale degree to letter conversion
        return `${letterNote} ${chordFunctionSuffix}`; // SPACE after letter!
    } else {
        // Scale degree mode: no space
        return `${scaleDegree}${chordFunctionSuffix}`;
    }
}

function makeNVXDiamondFace(scaleDegree, faceIndex, color, rotateDeg = 0, transparentBg = false, chordRoman = '') {
    /**
     * Creates cube face texture using NVX Diamond Font ligatures
     * Preserves exact sizing and orientation from makeCircleDiamondFace
     * @param {string} scaleDegree - Scale degree (e.g., '1', 'b7', '#4')
     * @param {number} faceIndex - Face position (0=root, 1=3rd, 2=5th, 3=7th)
     * @param {string} color - Circle color (e.g., '#2ecc71')
     * @param {number} rotateDeg - Rotation in degrees
     * @param {boolean} transparentBg - Use transparent background
     * @param {string} chordRoman - Roman numeral for chord context (e.g., 'vi', 'ii')
     * @returns {THREE.MeshStandardMaterial} Material with NVX font texture
     */

    const size = 512; // Match original canvas size
    const canvas = document.createElement('canvas');
    canvas.width = size;
    canvas.height = size;
    const ctx = canvas.getContext('2d');

    // Preserve exact rotation logic from original
    ctx.translate(size / 2, size / 2);
    ctx.rotate((rotateDeg * Math.PI) / 180);
    ctx.translate(-size / 2, -size / 2);

    // Use lightened wood background (preserve exact color)
    if (!transparentBg) {
        ctx.fillStyle = '#e8d4b8'; // Exact match from original
        ctx.fillRect(0, 0, size, size);
    }

    // Generate NVX ligature text with chord context
    const nvxText = generateNVXLigature(scaleDegree, faceIndex, (labelMode === 'letters'), currentKey, chordRoman);

    console.log(`[NVX DIAMOND] Face ${faceIndex}: ${scaleDegree} → "${nvxText}"`);

    // NVX Diamond Font handles ALL coloring automatically through ligatures!
    // DO NOT set ctx.fillStyle - the font ligatures create colored circles automatically!
    // Examples: "77th" = light gray circle, "b7b7th" = medium gray, "bb7bb7th" = dark gray

    // 📐 NVX FONT SIZE OPTIMIZATION - CALCULATED FROM USER IMAGE ANALYSIS 📐
    // Original was too small (~15% of face), user wants "as close to edges as possible"
    // V1.075: User feedback - "close, try 1/3rd bigger still"  
    // V1.076: User feedback - "nooe keep going! another 1/3rd of its size bigger!"
    // V1.077: User feedback - "keep going...." - EXTREME SIZE TEST!
    // V1.078: User feedback - "looks the same size. multiply by 2"
    // V1.079: User feedback - "now i see a change, that was obviously too big, reduce proportionally"
    // V1.080: User feedback - "good! raise the size a bit now, and the center anchor point needs to go down"
    let fontSize = 1650; // Increased from 1500px to 1650px (+10% size boost)
    const diamondSize = size * 0.64; // Match original diamond area (327px)
    const safeTextArea = size * 0.85; // Use 85% of FULL CANVAS (435px) not just diamond area

    // Check if NVX Diamond Font is loaded
    const nvxFontLoaded = document.fonts.check('16px "NVX Diamond Font"');
    const fontStack = nvxFontLoaded
        ? '"NVX Diamond Font", "Arial Unicode MS", system-ui'
        : '"Arial Unicode MS", system-ui'; // Fallback

    if (!nvxFontLoaded) {
        console.warn(`[NVX DIAMOND] Font not loaded, using fallback for: ${nvxText}`);
    }

    // Measure and adjust text size (preserve exact scaling logic)
    ctx.font = `700 ${fontSize}px ${fontStack}`;
    let textMetrics = ctx.measureText(nvxText);
    let textWidth = textMetrics.width;

    // Scale down if text is too wide (match original logic)
    let iterations = 0;
    while (textWidth > safeTextArea && iterations < 5) {
        fontSize = Math.floor(fontSize * 0.85); // Match original scaling factor
        ctx.font = `700 ${fontSize}px ${fontStack}`;
        textMetrics = ctx.measureText(nvxText);
        textWidth = textMetrics.width;
        iterations++;
    }

    // Center text positioning with vertical adjustment for NVX Diamond Font
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';

    // NVX Diamond Font renders with automatic ligature coloring - use black as base
    // The font's ligatures will override this with their own colored circles
    ctx.fillStyle = '#000000'; // Black base - ligatures create their own colored backgrounds

    // NVX Diamond Font vertical centering adjustment - move text down slightly
    const verticalOffset = fontSize * 0.05; // 5% of font size offset downward
    ctx.fillText(nvxText, size / 2, (size / 2) + verticalOffset);

    // Create Three.js texture with exact same properties
...
function makeCircleDiamondFace(text, color, rotateDeg = 0, transparentBg = false) {
    ...
    // Use lightened wood background (40% lighter for legibility)
    if (!transparentBg) {
        ctx.fillStyle = '#e8d4b8'; // Lightened wood color (was #f0e6d8)
        ctx.fillRect(0, 0, size, size);
    }
    ...
    // ENHANCED TEXT SIZING for diamond faces - ensure text fits within diamond
    ctx.fillStyle = '#111';
    const pretty = toMusicalGlyphs(text);

    // Responsive text sizing for diamond center
    let fontSize = 160; // Base size
    const diamondSize = size * 0.64; // Size of white diamond area
    const safeTextArea = diamondSize * 0.6; // Text should use 60% of diamond area

    // Measure and adjust text size
    ctx.font = `700 ${fontSize}px "Bravura Text", "Noto Music", "Arial Unicode MS", system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
    let textMetrics = ctx.measureText(pretty);
    let textWidth = textMetrics.width;

    // Scale down if text is too wide for diamond
    let iterations = 0;
    while (textWidth > safeTextArea && iterations < 5) {
        fontSize = Math.floor(fontSize * 0.85);
        ctx.font = `700 ${fontSize}px "Bravura Text", "Noto Music", "Arial Unicode MS", system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
        textMetrics = ctx.measureText(pretty);
        textWidth = textMetrics.width;
        iterations++;
    }
...
function makeExtensionBackFace(romanLabel) {
    ...
    // LIGHTENED Wood background for better legibility
    ctx.fillStyle = '#e8d4b8'; // 40% lighter wood color (was #f0e6d8)
    ctx.fillRect(0, 0, size, size);
...
async function makeMaterials(label, romanLabel, extensions = null) {
    ...
    const wood = new THREE.MeshStandardMaterial({ color: 0xe8d4b8 }); // 40% lighter wood color for better text legibility (Item #23)
    ...
    // 🎵 NVX DIAMOND FONT SYSTEM V1.62 - PRESERVING EXACT ORIENTATION 🎵
    console.log(`[NVX MATERIALS] Creating materials for ${romanLabel}, display: [${display.join(', ')}]`);

    // NVX Diamond Font faces with exact same rotation as original + chord context
    const faceBottom = makeNVXDiamondFace(display[0], 0, '#2ecc71', 0, false, romanLabel);     // root face (index 0)
    const faceRight = makeNVXDiamondFace(display[1], 1, '#e74c3c', 270, false, romanLabel);    // 3rd face (index 1)
    const faceTop = makeNVXDiamondFace(display[2], 2, '#3498db', 180, false, romanLabel);      // 5th face (index 2)
    const faceLeft = makeNVXDiamondFace(display[3], 3, '#bdc3c7', 90, false, romanLabel);      // 7th face (index 3)

    // COMMENTED OUT: Original graphics system (preserved for fallback)
        ...
        if (res.isClick) {
            ...
            if (targetObj.userData?.isShelf) {
                if (adjustMode) {
                    ...
                    // FIXED: Only Shift adds 7th (NOT Alt/Option - Option is for compound intervals)
                    console.log(`🚨 [SHIFT DETECT ADJUST] *** CHECKING SHIFT KEY STATE ***`);
                    console.log(`🚨 [SHIFT DETECT ADJUST] globalModifierState.shiftPressed: ${globalModifierState.shiftPressed}`);
                    console.log(`🚨 [SHIFT DETECT ADJUST] e.shiftKey: ${e.shiftKey}`);
                    const isShiftClick = globalModifierState.shiftPressed || e.shiftKey;
                    console.log(`🚨 [SHIFT DETECT ADJUST] isShiftClick (FINAL): ${isShiftClick}`);
                    const shouldUse7th = withSeventh || isShiftClick;

                    console.log(`[ADJUST DEBUG] *** SHIFT CHECK *** isShiftClick=${isShiftClick} (global=${globalModifierState.shiftPressed})`);

                    if (isShiftClick) {
                        console.log(`🚨 [ADJUST SHIFT+CLICK] *** EXECUTING SHIFT+CLICK LOGIC *** FORCING 7th for ${targetObj.userData.roman}`);
                        updateChordFaceWithDiatonic7th(targetObj);
                    } else {
                        console.log(`[ADJUST SHIFT+CLICK] *** NO SHIFT DETECTED *** Skipping 7th face update for ${targetObj.userData.roman}`);
                    }
            ...
            if (centerHit) {
                ...
                // FIXED: Only Shift adds 7th (NOT Alt/Option - Option is for compound intervals)
                console.log(`🚨 [SHIFT DETECT CENTER] *** CHECKING SHIFT KEY STATE ***`);
                console.log(`🚨 [SHIFT DETECT CENTER] globalModifierState.shiftPressed: ${globalModifierState.shiftPressed}`);
                console.log(`🚨 [SHIFT DETECT CENTER] e.shiftKey: ${e.shiftKey}`);
                const isShiftClick = globalModifierState.shiftPressed || e.shiftKey;
                console.log(`🚨 [SHIFT DETECT CENTER] isShiftClick (FINAL): ${isShiftClick}`);
                const shouldUse7th = withSeventh || isShiftClick;
                ...
                console.log(`[CLICK DEBUG] *** SHIFT CHECK *** isShiftClick=${isShiftClick} (global=${globalModifierState.shiftPressed} OR event=${e.shiftKey})`);

                if (isShiftClick) {
                    console.log(`🚨 [MODIFIER+CLICK] *** EXECUTING SHIFT+CLICK LOGIC *** FORCING 7th for ${targetObj.userData.roman}`);
                    // Update front face texture to show 7th notation temporarily
                    updateChordFaceWithDiatonic7th(targetObj);
                } else {
                    console.log(`[MODIFIER+CLICK] *** NO SHIFT DETECTED *** Skipping 7th face update for ${targetObj.userData.roman}`);
                }
            ...
            if (hit) {
                ...
                if (Math.abs(normalZ - 1) < 0.5) {
                    ...
                    // BULLETPROOF MODIFIER DETECTION - Use global state + event state
                    // FIXED: Only Shift adds 7th (NOT Alt/Option - Option is for compound intervals)
                    console.log(`🚨 [SHIFT DETECT QUAD] *** CHECKING SHIFT KEY STATE ***`);
                    console.log(`🚨 [SHIFT DETECT QUAD] globalModifierState.shiftPressed: ${globalModifierState.shiftPressed}`);
                    console.log(`🚨 [SHIFT DETECT QUAD] e.shiftKey: ${e.shiftKey}`);
                    const isShiftClick = globalModifierState.shiftPressed || e.shiftKey;
                    console.log(`🚨 [SHIFT DETECT QUAD] isShiftClick (FINAL): ${isShiftClick}`);
                    const shouldUse7th = withSeventh || isShiftClick;
                    ...
                    console.log(`[QUADRANT DEBUG] *** SHIFT CHECK *** isShiftClick=${isShiftClick} (global=${globalModifierState.shiftPressed} OR event=${e.shiftKey})`);

                    if (isShiftClick) {
                        console.log(`🚨 [MODIFIER+CLICK] *** EXECUTING SHIFT+CLICK LOGIC *** FORCING 7th for ${targetObj.userData.roman} (quadrant)`);
                        // Update front face texture to show 7th notation temporarily
                        updateChordFaceWithDiatonic7th(targetObj);
                    } else {
                        console.log(`[MODIFIER+CLICK] *** NO SHIFT DETECTED *** Skipping 7th face update for ${targetObj.userData.roman} (quadrant)`);
                    }
                    ...
                    if (voice === 'bass') {
                        ...
                        // HARD VOICE RANGE LIMITS: Apply constraints to prevent extreme ranges (PHOENIX-CLAUDE TODO #5)
                        midi = constrainToVoiceRange(midi, 'bass');
                        ...
                        if (window.audioEngine && window.audioEngine.playBass) {
                            ...
                            lastBassMidi = midi;
                            console.log('[CHORD CLICK] 🎵 Playing bass with VL3 + range limits:', bassNote, 'from', context.previousChord, '→', context.currentChord);
                        } else {
                        ...
                        // HARD VOICE RANGE LIMITS: Apply constraints to prevent extreme ranges (PHOENIX-CLAUDE TODO #5)
                        midi = constrainToVoiceRange(midi, 'melody');
                        ...
                        if (window.audioEngine && window.audioEngine.playMelody) {
                            ...
                            lastMelodyMidi = midi;
                            console.log('[CHORD CLICK] 🎵 Playing melody with VL3 + range limits:', melodyNote, 'from', context.previousChord, '→', context.currentChord);
                        } else {
...
// 🚨 DEBUG: Test if everything is available after load
console.log('🚨 [LOAD DEBUG] noteSetsC available:', !!noteSetsC);
console.log('🚨 [LOAD DEBUG] noteSetsC.I:', noteSetsC?.I);
console.log('🚨 [LOAD DEBUG] updateChordFaceWithDiatonic7th function exists:', typeof updateChordFaceWithDiatonic7th);

// Test function that can be called from browser console
window.testShift7th = function () {
    console.log('🚨 [TEST] Testing shift+7th function...');
    try {
        const fakeTarget = {
            userData: { roman: 'I' },
            material: [null, null, null, null, null, { map: null, needsUpdate: false }]
        };
        console.log('🚨 [TEST] Calling updateChordFaceWithDiatonic7th with fake I chord...');
        updateChordFaceWithDiatonic7th(fakeTarget);
...
// HARD VOICE RANGE LIMITS - Prevent extreme ranges during loop playback (PHOENIX-CLAUDE TODO #5)
const VOICE_RANGE_LIMITS = {
    bass: {
        min: 28,    // E1 - Absolute minimum for bass instrument
        max: 55,    // G3 - Keep bass in proper register 
        preferred_min: 33,  // A1 - Preferred minimum
        preferred_max: 48   // C3 - Preferred maximum for musical bass lines
    },
    melody: {
        min: 48,    // C3 - Minimum for melody to avoid bass register collision
        max: 84,    // C6 - Maximum playable on most instruments
        preferred_min: 60,  // C4 - Middle C as preferred minimum
        preferred_max: 79   // G5 - Preferred maximum for singable melodies
    }
};

// Apply hard voice range constraints to MIDI notes
function constrainToVoiceRange(midi, voice) {
    if (!VOICE_RANGE_LIMITS[voice]) return midi;

    const limits = VOICE_RANGE_LIMITS[voice];
    const targetPitchClass = midi % 12;

    // If already within range, use as-is
    if (midi >= limits.min && midi <= limits.max) {
        return midi;
    }

    console.log(`[VOICE LIMITS] ⚠️ ${voice.toUpperCase()} note ${midi} outside range [${limits.min}-${limits.max}], constraining...`);

    // Find the best octave placement for this pitch class within the allowed range
    let candidates = [];
    ...
    if (candidates.length === 0) {
        // Fallback: force into range even if not ideal
        console.warn(`[VOICE LIMITS] ❌ No valid octave for ${voice} pitch class ${targetPitchClass}, using range bounds`);
        return midi < limits.min ? limits.min : limits.max;
    }

    // Choose the candidate closest to the original midi value, but prefer the comfortable range
    const preferredCandidates = candidates.filter(c =>
        c >= limits.preferred_min && c <= limits.preferred_max
    );

    const finalCandidates = preferredCandidates.length > 0 ? preferredCandidates : candidates;
    const bestCandidate = finalCandidates.reduce((best, candidate) =>
        Math.abs(candidate - midi) < Math.abs(best - midi) ? candidate : best
    );

    console.log(`[VOICE LIMITS] ✅ ${voice.toUpperCase()} constrained: ${midi} → ${bestCandidate} (pitch class ${targetPitchClass})`);
...
class OrchestralAudioEngine {
    ...
    // ADD MISSING INIT METHOD WITH SPAM PREVENTION
    async init() {
        // 🚨 SPAM PREVENTION: Only initialize once
        if (this.isReady) {
            console.log('[AUDIO ENGINE] 🔒 Already initialized - preventing spam');
            return;
        }
        if (this.isInitializing) {
            console.log('[AUDIO ENGINE] 🔒 Already initializing - preventing duplicate');
            return;
        }

        this.isInitializing = true;
        console.log('[AUDIO ENGINE] 🔧 Init method called - starting background loading');

        try {
            await this.loadRealInstrumentsInBackground();
            this.isReady = true;
            console.log('[AUDIO ENGINE] ✅ Init method completed successfully');
        } catch (error) {
            console.error('[AUDIO ENGINE] 💀 Init method failed:', error);
        } finally {
            this.isInitializing = false;
        }
    }
    ...
    async loadRealOrchestralSamplers() {
        ...
        // 🚨 SPAM PREVENTION: Only load samples once
        if (this.realInstruments && Object.keys(this.realInstruments).length > 0) {
            console.log('[AUDIO ENGINE] 🔒 Samples already loaded - preventing spam reload');
            return;
        }
...
async function loadInstruments() {
    ...
    // 🚨 SPAM PREVENTION: Only initialize once
    if (window.audioEngine && (window.audioEngine.isReady || window.audioEngine.isInitializing)) {
        console.log('[MAIN] 🔒 Audio engine already initialized/initializing - preventing spam');
        return;
    }
    ...
    // 🚨 EMERGENCY FIX: Register audio engine for disposal
    disposalManager.register(window.audioEngine);
    console.log('[MEMORY LEAK FIX] ✅ Audio engine registered for disposal');
...
menuLockMelodyBtn?.addEventListener('click', () => {
    try {
        // Update transport state
        if (window.chordCubesTransport) {
            window.chordCubesTransport.lockMelody = true;
        }
        lockInMelody();
        setMelodyLockVisual('closed');
    } catch (err) {
        console.error('[MENU] Error locking melody:', err);
    }
});
menuUnlockMelodyBtn?.addEventListener('click', () => {
    try {
        // Update transport state
        if (window.chordCubesTransport) {
            window.chordCubesTransport.lockMelody = false;
        }
        lockedMelody = null;
        renderMelodyLane();
        setMelodyLockVisual('open');
    } catch (err) {
        console.error('[MENU] Error unlocking melody:', err);
    }
});
menuLockBassBtn?.addEventListener('click', () => {
    try {
        // Update transport state
        if (window.chordCubesTransport) {
            window.chordCubesTransport.lockBass = true;
        }
        lockInBass();
        setBassLockVisual('closed');
    } catch (err) {
        console.error('[MENU] Error locking bass:', err);
    }
});
menuUnlockBassBtn?.addEventListener('click', () => {
    try {
        // Update transport state
        if (window.chordCubesTransport) {
            window.chordCubesTransport.lockBass = false;
        }
        lockedBass = null;
        renderBassLane();
        setBassLockVisual('open');
    } catch (err) {
        console.error('[MENU] Error unlocking bass:', err);
    }
});
...
playProgressionBtnMain?.addEventListener('click', async () => {
    console.log('[UI PLAY PROGRESSION] Button clicked!');

    // Make sure UI shows the button is active
    if (playProgressionBtnMain) {
        playProgressionBtnMain.style.background = '#555';
        setTimeout(() => {
            if (playProgressionBtnMain) playProgressionBtnMain.style.background = '#333';
        }, 150);
    }

    // Strong audio initialization - we need multiple methods to ensure it works across browsers
    try {
        console.log('[UI PLAY PROGRESSION] Starting audio initialization...');

        // Method 1: Via transport bridge (our preferred method)
        if (window.chordCubesTransport) {
            const success = await window.chordCubesTransport.ensureAudioContext();
            console.log('[UI PLAY PROGRESSION] Audio context initialized via transport:', success);

            // If that fails, try direct Tone.js method
            if (!success && window.Tone && window.Tone.context) {
                console.log('[UI PLAY PROGRESSION] Transport init failed, trying direct Tone.js init...');
                await window.Tone.start();
                console.log('[UI PLAY PROGRESSION] Direct Tone.js initialization successful');
            }
        } else {
            // Fallback if transport isn't available
            if (window.Tone && window.Tone.context) {
                console.log('[UI PLAY PROGRESSION] No transport available, using direct Tone.js init...');
                if (window.Tone.context.state !== 'running') {
                    await window.Tone.start();
                    console.log('[UI PLAY PROGRESSION] Tone.js context started directly');
                }
            }
        }

        // Method 3: Force unlock with a silent sound - BROWSER HACK
        try {
            // Only attempt if we have user gesture (prevents AudioContext spam)
            if (document.hasStoredUserActivation !== false) {
                ...
                console.log('[UI PLAY PROGRESSION] Silent oscillator hack completed');
            }
        } catch (silentErr) {
            console.warn('[UI PLAY PROGRESSION] Silent oscillator hack failed:', silentErr);
        }

        // CRITICAL: Try a DIRECT play approach if there have been issues with the sequence
        if (window.directPlayEnabled) {
            console.log('[UI PLAY PROGRESSION] Using direct play approach (bypassing sequence)');
            try {
                await directPlayProgression();
                return; // Exit early if we used the direct approach
            } catch (directErr) {
                console.error('[UI PLAY PROGRESSION] Direct play failed:', directErr);
                // Fall back to normal method
            }
        }
    } catch (err) {
        console.error('[UI PLAY PROGRESSION] Audio initialization failed:', err);
    }

    // Play progression using standard approach
    try {
        console.log('[UI PLAY PROGRESSION] Calling playFrontRowProgression()...');
        await playFrontRowProgression();
    } catch (err) {
        console.error('[UI PLAY PROGRESSION] Error in playFrontRowProgression:', err);

        // If standard approach fails, try direct play as fallback
        console.log('[UI PLAY PROGRESSION] Standard approach failed, trying direct play fallback...');
        try {
            // Mark direct play as enabled for future clicks
            window.directPlayEnabled = true;
            await directPlayProgression();
        } catch (directErr) {
            console.error('[UI PLAY PROGRESSION] Direct play fallback also failed:', directErr);
        }
    }
});

// Direct play function that bypasses sequence-based approach for maximum reliability
async function directPlayProgression() {
    console.log('[DIRECT PLAY] Starting direct play approach');

    if (lineup.length === 0) {
        console.log('[DIRECT PLAY] No lineup found, returning');
        return;
    }
    ...
    // 🎼 STAVES INTEGRATION: Notify staves that playback has started
    ...
    // Reset tracking variables for voice leading
    lastProgressionBassMidi = null;
    lastProgressionMelodyMidi = null;

    // First, make sure audio engine is initialized
    if (window.audioEngine && !window.audioEngine.isReady && window.audioEngine.init) {
        try {
            await window.audioEngine.init();
            console.log('[DIRECT PLAY] Successfully initialized audio engine');
        } catch (err) {
            console.error('[DIRECT PLAY] Error initializing audio engine:', err);
        }
    } else if (window.audioEngine && window.audioEngine.isReady) {
        console.log('[DIRECT PLAY] Audio engine already ready - skipping init');
    }

    // Play each chord with a delay
    const totalChords = lineup.length * loopCount;
    console.log(`[DIRECT PLAY] Playing ${lineup.length} chords x ${loopCount} loops = ${totalChords} total`);

    const BPM = window.drumMachine?.bpm || 120; // Use drum machine BPM if available
    ...
    if (window.drumMachine && !window.drumMachine.isPlaying) {
        try {
            console.log('[DIRECT PLAY] Starting drum machine');
            window.drumMachine.toggleDrums();
        } catch (err) {
            console.warn('[DIRECT PLAY] Error starting drum machine:', err);
        }
    }

    // Create an array of timeout IDs for cleanup
    const timeoutIds = [];

    // More precise timing with performance.now()
    const startTime = performance.now();

    for (let i = 0; i < totalChords; i++) {
        const chordIndex = i % lineup.length;
        const c = lineup[chordIndex];
        const chordRoman = c.userData?.roman || 'I';
        const currentLoop = Math.floor(i / lineup.length) + 1;
        const chordInLoop = chordIndex + 1;

        // Calculate precise timing
        const targetTime = startTime + (i * msPerMeasure);

        // Use setTimeout to schedule each chord with more precise timing
        const timeoutId = setTimeout(() => {
            try {
                // Calculate timing drift for logging purposes
                const actualTime = performance.now();
                const drift = actualTime - (startTime + (i * msPerMeasure));

                console.log(`[DIRECT PLAY] Loop ${currentLoop}/${loopCount}, Chord ${chordInLoop}/${lineup.length}: ${chordRoman} (drift: ${drift.toFixed(2)}ms)`);

                // Visual effects
                highlightChordEffect(c, 900);
                pulseGiantAt(chordIndex, 700);

                // Camera dolly similar to sequence version
                const tDur = 700;
                ...
                // Play the chord with audio engine
                const chordMidis = buildLockedChordBedMidis(chordRoman, false);
                const chordNotes = chordMidis.map(midi => Tone.Frequency(midi, "midi").toNote());

                if (window.audioEngine) {
                    // 🔇 CRITICAL: Cut off previous chord before playing new one to prevent bleeding
                    ...
                    if (bassEnabled) {
                        ...
                        if (lockedBass && lockedBass[chordIndex]?.midi) {
                            bassMidi = lockedBass[chordIndex].midi;
                        } else {
                            bassMidi = getBassMidiForObject(c);
                        }
                        ...
                        // Voice leading
                        if (i > 0 && lastProgressionBassMidi !== null) {
                            bassMidi = voiceLeadMidi(bassMidi, lastProgressionBassMidi, {
                                voice: 'bass',
                                currentChord: chordRoman,
                                isProgression: true
                            });
                        }
                        ...
                        window.audioEngine.playBass(bassNote, secondsPerMeasure * 0.95, 0.8);
                    }
                    ...
                    if (melodyEnabled) {
                        ...
                        if (lockedMelody && lockedMelody[chordIndex]?.midi) {
                            melMidi = lockedMelody[chordIndex].midi;
                        } else {
                            melMidi = getMelodyMidiForObject(c);
                        }
                        ...
                        // Voice leading
                        if (i > 0 && lastProgressionMelodyMidi !== null) {
                            melMidi = voiceLeadMidi(melMidi, lastProgressionMelodyMidi, {
                                voice: 'melody',
                                currentChord: chordRoman,
                                isProgression: true
                            });
                        ...
                        window.audioEngine.playMelody([melodyNote], [secondsPerMeasure * 0.95], 0.5);
                    }
                }
            } catch (err) {
                console.error('[DIRECT PLAY] Error playing chord:', err);
            }
        }, Math.max(0, targetTime - performance.now())); // Account for elapsed time

        // Store timeout ID for cleanup
        timeoutIds.push(timeoutId);
    }

    // Schedule cleanup after playback is complete
    const finalTimeoutId = setTimeout(() => {
        try {
            // Reset view and lighting
            setViewAbove();
            ambient.intensity = 0.7;
            dir.intensity = 0.7;
            // Apply performance lighting if in performance mode
            applyPerformanceLighting();
            ...
            // Stop drum machine if it was started
            if (window.drumMachine && window.drumMachine.isPlaying) {
                try {
                    window.drumMachine.toggleDrums();
                } catch (err) {
                    console.warn('[DIRECT PLAY] Error stopping drum machine:', err);
                }
            }

            console.log('[DIRECT PLAY] ✅ Playback complete, reset successful');
        } catch (err) {
            console.error('[DIRECT PLAY] Error during reset:', err);
        }
    }, startTime + (totalChords * msPerMeasure) + 1000 - performance.now());

    timeoutIds.push(finalTimeoutId);

    // Add emergency stop function to emergency stop button
    const emergencyStopBtn = document.getElementById('emergency-stop-btn');
    if (emergencyStopBtn) {
        const originalClick = emergencyStopBtn.onclick;
        emergencyStopBtn.onclick = function () {
            console.log('[DIRECT PLAY] Emergency stop triggered');
            ...
            if (window.drumMachine && window.drumMachine.isPlaying) {
                try {
                    ...
                    console.warn('[DIRECT PLAY] Error stopping drum machine during emergency stop:', err);
                }
            }
            ...
            // Call original click handler if it existed
...
emergencyStopBtn?.addEventListener('click', () => {
    ...
    // 🎼 STAVES INTEGRATION: Notify staves that playback has stopped
    try {
        if (window.musicalStaves3D) {
            window.musicalStaves3D.setPlaybackActive(false);
            console.log('[EMERGENCY STOP] 🎼 Staves notified of emergency stop');
        }
    } catch (e) { console.log('[EMERGENCY STOP] ⚠️ Staves notification failed:', e); }
...
function setupLockButtons() {
    ...
    lockBassBtn?.addEventListener('click', () => {
        console.log('[UI] Lock bass button clicked');
        const bassVolume = document.getElementById('bass-volume');
        ...
        if (currentVolume > 0) {
            // LOCK BASS
            try {
                // First update transport state for playback
                if (window.chordCubesTransport) {
                    window.chordCubesTransport.lockBass = true;
                }

                // Then capture the actual notes
                lockInBass();
                setBassLockVisual('closed');
                lastBassVolume = currentVolume;
                console.log('[SACRED LOCK] Bass locked');
            } catch (err) {
                console.error('[UI] Error locking bass:', err);
            }
        } else {
            // UNLOCK BASS
            try {
                // First update transport state
                if (window.chordCubesTransport) {
                    window.chordCubesTransport.lockBass = false;
                }
                ...
                console.error('[UI] Error unlocking bass:', err);
            }
        }

        bassEnabled = isBassEnabled();
        updateLockButtonStyle(lockBassBtn, currentVolume > 0);
    });

    // Melody lock button - CALLS EXACT SACRED FUNCTION
    let lastMelodyVolume = 0.05;
    lockMelodyBtn?.addEventListener('click', () => {
        console.log('[UI] Lock melody button clicked');
        const melodyVolume = document.getElementById('melody-volume');
        ...
        if (currentVolume > 0) {
            ...
            try {
                ...
                console.log('[SACRED LOCK] Melody locked');
            } catch (err) {
                console.error('[UI] Error locking melody:', err);
            }
        } else {
            // UNLOCK MELODY
            try {
                ...
                console.error('[UI] Error unlocking melody:', err);
            }
        }

        melodyEnabled = isMelodyEnabled();
        updateLockButtonStyle(lockMelodyBtn, currentVolume > 0);
    });
}
...
// CHROMATIC CHORD EXTENSION SYSTEM - MOVED ABOVE EVENT LISTENER V1.62  
...
document.addEventListener('keydown', async (e) => {
    ...
    // ENHANCED SHIFT 7TH MODE V1.61 - More responsive detection
    // Trigger on any shift key state, not just isolated Shift key press
    if (e.shiftKey && !e.altKey && !e.metaKey && !e.ctrlKey) {
        // Check if shift state just changed to pressed
        const wasShiftPressed = globalModifierState.shiftPressed;

        if (!wasShiftPressed) {
            console.log(`[7TH FIX V1.61] 🎵 Shift detected (key: ${key}) - enabling 7th mode for ALL eligible chords`);

            // ENHANCED: Ensure global state is updated immediately
            globalModifierState.shiftPressed = true;

            // Force refresh with 7th preview - ENHANCED LOGGING
            console.log(`[7TH FIX V1.61] 🔄 Triggering refreshAllCubeFaces(true) with enhanced exclusion logic`);
            await refreshAllCubeFaces(true); // previewWith7th = true

            console.log(`[7TH FIX V1.61] ✅ Successfully applied 7th preview to ALL eligible cube faces`);

            // Don't preventDefault if this is a key combination (allow other handlers)
            if (key === 'Shift') {
                e.preventDefault();
            }
            return;
...
document.addEventListener('keyup', async (e) => {
    ...
    // ENHANCED SHIFT RELEASE V1.61 - Responsive 7th mode exit
    if (key === 'Shift' || (!e.shiftKey && globalModifierState.shiftPressed)) {
        console.log(`[7TH FIX V1.61] 🎵 Shift released (key: ${key}) - exiting 7th mode`);

        // ENHANCED: Ensure global state is updated immediately
        globalModifierState.shiftPressed = false;

        // Force refresh without 7th preview - ENHANCED LOGGING
        console.log(`[7TH FIX V1.61] 🔄 Triggering refreshAllCubeFaces(false) to remove 7th preview`);
        await refreshAllCubeFaces(false); // previewWith7th = false

        console.log(`[7TH FIX V1.61] ✅ Successfully removed 7th preview from ALL cube faces`);
        return;
    }
...
function applyChordExtensions(baseNotes, rootNote, extensions) {
    ...
    // V1.62 FIX: Detect '2' as sus2 and '4' as sus4 (Priority 4)
    const hasSus2 = Array.from(extensions).some(ext => ext.name === 'sus2' || ext.name === '2');
    const hasSus4 = Array.from(extensions).some(ext => ext.name === 'sus4' || ext.name === '4');

    if (hasSus2 || hasSus4) {
        console.log(`[SUS CHORD V1.62] 🎵 Sus chord detected! Sus2: ${hasSus2}, Sus4: ${hasSus4}`);
        console.log(`[SUS CHORD V1.62] 🎵 Original chord notes: ${extendedNotes.join(', ')}`);
        ...
        console.log(`[SUS CHORD V1.62] 🎵 Will remove 3rds: Major=${major3rd}, Minor=${minor3rd}`);
        ...
        console.log(`[SUS CHORD V1.62] 🎵 After removing 3rds: ${extendedNotes.join(', ')}`);
        console.log(`[SUS CHORD V1.62] 🎵 Sus chord audio ready - 3rd removed, ${hasSus2 ? '2nd' : '4th'} will be added`);
    }
...
resetBtn?.addEventListener('click', () => {
    ...
    try {
        if (window.musicalStaves3D) {
            window.musicalStaves3D.setPlaybackActive(false);
            console.log('[ENHANCED RESET] 🎼 Staves notified of reset stop');
        }
    } catch (e) { console.log('[ENHANCED RESET] ⚠️ Staves notification failed:', e); }
...
function ensureAudio() {
    if (!audioCtx) {
        try {
            // Only create AudioContext if we likely have user gesture
            if (document.hasStoredUserActivation !== false) {
                audioCtx = new (window.AudioContext || window.webkitAudioContext)();
            } else {
                console.log('[AUDIO] Deferring AudioContext creation until user gesture');
                return null;
            }
        } catch (err) {
            console.log('[AUDIO] AudioContext creation failed (no user gesture)');
            return null;
        }
    }
    return audioCtx;
...
function playChordForObjectWith7th(obj, use7th = false, options = {}) {
    ...
    if (isBassEnabled()) {
        ...
        // HARD VOICE RANGE LIMITS: Apply constraints to prevent extreme ranges (PHOENIX-CLAUDE TODO #5)
        bassMidi = constrainToVoiceRange(bassMidi, 'bass');
    ...
    if (isMelodyEnabled()) {
        ...
        // HARD VOICE RANGE LIMITS: Apply constraints to prevent extreme ranges (PHOENIX-CLAUDE TODO #5)
        melMidi = constrainToVoiceRange(melMidi, 'melody');
...
function midiToNoteNameNVX(midiNumber) {
    /**
     * Converts MIDI number to note name with NVX-preferred accidentals
     * @param {number} midiNumber - MIDI note number (0-127)  
     * @returns {string} Note name with flats (e.g., "C", "F#", "Bb")
     */
...
function makeLaneNodeNVX(roman, colorHex, midiNumber) {
    /**
     * Creates lane node using NVX Diamond Font system for note display
     * @param {string} roman - Roman numeral for context
     * @param {string} colorHex - Background circle color
     * @param {number} midiNumber - MIDI note number to display
     * @returns {THREE.Group} Group with NVX font-rendered note
     */
    console.log(`[DEBUG NVX LANE] 🎨 Creating NVX lane node: ${roman}, MIDI: ${midiNumber}, Color: ${colorHex}`);
    const group = new THREE.Group();

    // Background circle (same as original)
    ...
    // Convert MIDI to note name using NVX-preferred accidentals
    const noteName = midiToNoteNameNVX(midiNumber);

    // Create NVX Diamond Font texture for the note name
    // Use letter mode (isLetterMode = true) with space after letter
    const nvxText = `${noteName} r`; // Note with root designation

    console.log(`[DEBUG NVX LANE] 📝 MIDI ${midiNumber} → ${noteName} → "${nvxText}"`);

    const size = 256; // Smaller canvas for lane nodes
    const canvas = document.createElement('canvas');
    canvas.width = size;
    canvas.height = size;
    const ctx = canvas.getContext('2d');

    // Add background for NVX Diamond Font visibility
    ctx.fillStyle = '#f0f0f0'; // Light gray background for contrast
    ctx.fillRect(0, 0, size, size);

    // NVX Diamond Font rendering - scaled for lane display
    let fontSize = 800; // Increased from 600px for better visibility
    const safeTextArea = size * 0.85;

    // Check if NVX Diamond Font is loaded
    const nvxFontLoaded = document.fonts.check('16px "NVX Diamond Font"');
    const fontStack = nvxFontLoaded
        ? '"NVX Diamond Font", "Arial Unicode MS", system-ui'
        : '"Arial Unicode MS", system-ui';

    console.log(`[DEBUG NVX LANE] 🔤 Font loaded: ${nvxFontLoaded}, Stack: ${fontStack}`);
    ...
    // Black base - NVX Diamond Font ligatures create their own colored backgrounds
    ctx.fillStyle = '#000000';

    // Vertical centering adjustment (same as cube faces)
    ...
    // Create plane to display the NVX font texture - LARGER for visibility
    const geometry = new THREE.PlaneGeometry(0.24, 0.24); // Increased from 0.16 for better visibility
    const material = new THREE.MeshBasicMaterial({
        map: texture,
        transparent: false, // Changed to false since we have background
        depthWrite: false,
        depthTest: false,
    ...
    textMesh.renderOrder = 10; // Higher render order to ensure visibility above everything
    textMesh.position.z = 0.01; // Slight offset to avoid z-fighting
    group.add(textMesh);

    console.log(`[DEBUG NVX LANE] ✅ Created NVX lane node: Circle + Text mesh (${geometry.parameters.width}x${geometry.parameters.height})`);

    group.userData = { roman, midiNumber, noteName };
    return group;
}
...
function renderMelodyLane() {
    ...
    for (let i = 0; i < lineup.length; i++) {
        ...
        // 🎨 NVX DIAMOND FONT UPGRADE: Use locked melody data if available
        let node;
        console.log(`[DEBUG RENDER MELODY] 🔍 Cube ${i}: lockedMelody=${!!lockedMelody}, length=${lockedMelody?.length}, data=${JSON.stringify(lockedMelody?.[i])}`);

        if (lockedMelody && lockedMelody[i]) {
            // Use NVX font system with actual MIDI note from locked melody
            const melodyData = lockedMelody[i];
            console.log(`[NVX MELODY] ✅ Creating NVX node for ${melodyData.roman}: MIDI ${melodyData.midi}`);
            node = makeLaneNodeNVX(melodyData.roman, melodyData.color, melodyData.midi);
        } else {
            // Fallback to old system if no locked melody data
            console.log(`[NVX MELODY] ❌ Using fallback old system for cube ${i}`);
            node = makeLaneNode(cube.userData.roman, color);
        }
...
function renderBassLane() {
    ...
    for (let i = 0; i < lineup.length; i++) {
        ...
        // 🎨 NVX DIAMOND FONT UPGRADE: Use locked bass data if available
        let node;
        console.log(`[DEBUG RENDER BASS] 🔍 Cube ${i}: lockedBass=${!!lockedBass}, length=${lockedBass?.length}, data=${JSON.stringify(lockedBass?.[i])}`);

        if (lockedBass && lockedBass[i]) {
            // Use NVX font system with actual MIDI note from locked bass
            const bassData = lockedBass[i];
            console.log(`[NVX BASS] ✅ Creating NVX node for ${bassData.roman}: MIDI ${bassData.midi}`);
            node = makeLaneNodeNVX(bassData.roman, bassData.color, bassData.midi);
        } else {
            // Fallback to old system if no locked bass data
            console.log(`[NVX BASS] ❌ Using fallback old system for cube ${i}`);
            node = makeLaneNode(cube.userData.roman, color);
        }
...
function lockInMelody() {
    ...
    // 🎼 STAVES INTEGRATION: Remove auto-show - staves only appear when user clicks "Show Staves"
    console.log('[LOCK MELODY] 🎼 Melody locked - staves will update if visible');
    if (window.musicalStaves3D && window.musicalStaves3D.isVisible) {
        console.log('[LOCK MELODY] 🎼 Updating existing visible staves');
...
function lockInBass() {
    ...
    // 🎼 STAVES INTEGRATION: Remove auto-show - staves only appear when user clicks "Show Staves"
    console.log('[LOCK BASS] 🎼 Bass locked - staves will update if visible');
    if (window.musicalStaves3D && window.musicalStaves3D.isVisible) {
        console.log('[LOCK BASS] 🎼 Updating existing visible staves');
        window.musicalStaves3D.drawMeasures(lineup.length);
    }
...
function playMelodyOnly() {
    ...
    if (window.musicalStaves3D) {
        window.musicalStaves3D.setPlaybackActive(true);
        window.musicalStaves3D.displayChordProgression(lineup.map(c => ({ roman: c.userData.roman })));
        console.log('[MELODY SOLO] 🎼 Staves notified of playback start');
    }
...
function playBassOnly() {
    ...
    if (window.musicalStaves3D) {
        window.musicalStaves3D.setPlaybackActive(true);
        window.musicalStaves3D.displayChordProgression(lineup.map(c => ({ roman: c.userData.roman })));
        console.log('[BASS SOLO] 🎼 Staves notified of playback start');
    }
    ...
    const soloSequence = new Tone.Sequence((time, index) => {
        ...
        if (soloType === 'melody') {
            if (lockedMelody) {
                ...
                // NEW AUDIO ENGINE: Use real melody instruments
                Tone.Draw.schedule(() => {
                    // 🔇 CRITICAL: Cut off previous notes before playing new melody to prevent bleeding
                    window.audioEngine.cutoffCurrentChord();
                ...
                Tone.Draw.schedule(() => {
                    ...
                    if (index > 0 && lastMelodyMidi != null) {
                        ...
                        // HARD VOICE RANGE LIMITS: Apply constraints to prevent extreme ranges (PHOENIX-CLAUDE TODO #5)
                        melMidi = constrainToVoiceRange(melMidi, 'melody');
                        console.log(`[MELODY SOLO] 🎼 VL3 + range limits result for face-derived melody: ${melMidi}`);
                    } else {
                        // First chord: apply basic register constraints
                        while (melMidi > 84) melMidi -= 12; while (melMidi < 60) melMidi += 12;
                    }
            ...
            if (lockedBass) {
                ...
                // NEW AUDIO ENGINE: Use real bass instruments
                Tone.Draw.schedule(() => {
                    // 🔇 CRITICAL: Cut off previous notes before playing new bass to prevent bleeding
                    window.audioEngine.cutoffCurrentChord();
                ...
                Tone.Draw.schedule(() => {
                    ...
                    if (index > 0 && lastBassMidi != null) {
                        ...
                        // HARD VOICE RANGE LIMITS: Apply constraints to prevent extreme ranges (PHOENIX-CLAUDE TODO #5)
                        bassMidi = constrainToVoiceRange(bassMidi, 'bass');
                        console.log(`[BASS SOLO] 🎼 VL3 + range limits result for face-derived bass: ${bassMidi}`);
                    } else {
                        // First chord: apply basic register constraints
                        while (bassMidi > 55) bassMidi -= 12; while (bassMidi < 36) bassMidi += 12;
                    }
...
async function playFrontRowProgression() {
    console.log('[PLAY PROGRESSION] Function called');

    // Force immediate audio initialization with all available methods
    try {
        // First check Tone.js
        if (window.Tone) {
            console.log('[PLAY PROGRESSION] Tone.js found, current state:', window.Tone.context?.state);
            if (window.Tone.context && window.Tone.context.state !== 'running') {
                console.log('[PLAY PROGRESSION] Starting Tone.js context...');
                try {
                    await window.Tone.start();
                    console.log('[PLAY PROGRESSION] Successfully started Tone.js context');
                } catch (err) {
                    console.warn('[PLAY PROGRESSION] Tone.start failed, trying alternative:', err);

                    if (window.Tone.context) {
                        await window.Tone.context.resume();
                        console.log('[PLAY PROGRESSION] Resumed Tone.js context, new state:', window.Tone.context.state);
                    }

                    // Create a silent sound as a browser hack to unlock audio
                    try {
                        // Only attempt if we likely have user gesture
                        if (document.hasStoredUserActivation !== false) {
                            const unlockContext = new (window.AudioContext || window.webkitAudioContext)();
                            const unlockOsc = unlockContext.createOscillator();
                            const unlockGain = unlockContext.createGain();
                            unlockGain.gain.value = 0.001; // Nearly silent
                            unlockOsc.connect(unlockGain);
                            unlockGain.connect(unlockContext.destination);
                            unlockOsc.start(0);
                            unlockOsc.stop(unlockContext.currentTime + 0.001);
                            console.log('[PLAY PROGRESSION] Executed audio context unlock hack');
                        }
                    } catch (unlockErr) {
                        console.warn('[PLAY PROGRESSION] Unlock hack failed:', unlockErr);
                    }
                }
            }

            // Force Tone.Transport to reset and start cleanly
            console.log('[PLAY PROGRESSION] Current Transport state:', Tone.Transport.state);
            if (Tone.Transport.state === "started") {
                console.log('[PLAY PROGRESSION] Stopping transport to ensure clean state');
                Tone.Transport.stop();
                // Small delay to ensure clean reset
                await new Promise(resolve => setTimeout(resolve, 100));
            }

            console.log('[PLAY PROGRESSION] Starting transport');
            Tone.Transport.start();
            console.log('[PLAY PROGRESSION] Transport started, state:', Tone.Transport.state);
        }

        // Also ensure audio via transport bridge
        if (window.chordCubesTransport) {
            console.log('[PLAY PROGRESSION] Using transport bridge to ensure audio');
            await window.chordCubesTransport.ensureAudioContext();
        }

        // Also try audio engine init if available
        if (window.audioEngine && !window.audioEngine.isReady && window.audioEngine.init) {
            console.log('[PLAY PROGRESSION] Initializing audio engine');
            await window.audioEngine.init();
        } else if (window.audioEngine && window.audioEngine.isReady) {
            console.log('[PLAY PROGRESSION] Audio engine already ready - skipping init');
        }

    } catch (err) {
        console.error('[PLAY PROGRESSION] Audio initialization error:', err);
    }

    // Check if we have any chords in the lineup
    if (lineup.length === 0) {
        console.log('[PLAY PROGRESSION] No lineup found, returning');
        return;
    }
    ...
    if (window.drumMachine && !window.drumMachine.isPlaying) {
        ...
        try {
            ...
            console.warn('[PLAY PROGRESSION] Error starting drum machine:', err);
        }
    }
    ...
    if (window.chordProgressionSequence) {
        try {
            window.chordProgressionSequence.dispose();
            window.chordProgressionSequence = null;
            console.log('[PLAY PROGRESSION] Disposed existing sequence');
        } catch (disposeErr) {
            console.warn('[PLAY PROGRESSION] Error disposing sequence:', disposeErr);
        }
    }
    ...
    try {
        const progressionSequence = new Tone.Sequence((time, index) => {
            try {
                if (index >= totalChords) {
                    ...
                    if (window.musicalStaves3D) {
                        window.musicalStaves3D.setPlaybackActive(false);
                        console.log('[TRANSPORT] 🎼 Staves notified of playback stop');
                    }
                    ...
                    try {
                        ...
                        console.error('[TRANSPORT] Error disposing sequence:', disposeErr);
                    }
                    ...
                    if (window.drumMachine && window.drumMachine.isPlaying) {
                        setTimeout(() => {
                            try {
                                ...
                                console.error('[TRANSPORT] Error stopping drum machine:', err);
                            }
                        }, 100);
                    }
                    ...
                    setTimeout(() => {
                        try {
                            ...
                            console.error('[TRANSPORT] Error during auto-reset:', resetErr);
                        }
                    }, 1000);
                ...
                if (isNaN(index) || index < 0) {
                    console.error('[TRANSPORT] Invalid index value:', index);
                    return;
                }

                // Safety check for lineup access
                if (!lineup || !Array.isArray(lineup) || lineup.length === 0) {
                    console.error('[TRANSPORT] Invalid lineup array');
                    return;
                }

                // Get the current chord from the lineup (with bounds checking)
                const chordIndex = index % lineup.length;
                if (chordIndex < 0 || chordIndex >= lineup.length) {
                    console.error('[TRANSPORT] Chord index out of bounds:', chordIndex);
                    return;
                }

                const c = lineup[chordIndex];
                if (!c) {
                    console.error('[TRANSPORT] Null chord at index:', chordIndex);
                    return;
                }
                ...
                const currentBpm = Tone.Transport.bpm.value || 120; // Default to 120 if undefined
                const beatsPerMeasure = 4; // Standard 4/4 time
                const chordDurationSeconds = (60 / currentBpm) * beatsPerMeasure; // Full measure duration

                console.log(`[TRANSPORT] Loop ${currentLoop}/${loopCount}, Chord ${chordInLoop}/${lineup.length}: ${c.userData?.roman || 'Unknown'} at time ${time?.toFixed(3) || 'unknown'}s, BPM: ${currentBpm}, sustain: ${chordDurationSeconds.toFixed(2)}s`);
                ...
                console.log(`[PLAY PROGRESSION] 🔥 Playing chord ${c.userData?.roman || 'Unknown'} with NEW AUDIO ENGINE`);
                ...
                Tone.Draw.schedule(() => {
                    try {
                        ...
                        if (window.audioEngine) {
                            console.log(`[PLAY PROGRESSION] 🎵 Playing ${c.userData?.roman || 'Unknown'} with real samples, duration: ${chordDurationSeconds.toFixed(2)}s`);

                            // Get chord notes
                            const chordMidis = buildLockedChordBedMidis(c.userData?.roman || 'I', withSeventh);
                            ...
                            if (bassEnabled && !lockedBass) {
                                ...
                                if (index > 0 && lastProgressionBassMidi !== null) {
                                    ...
                                    const context = {
                                        voice: 'bass',
                                        currentChord: c.userData?.roman || 'I',
                                        previousChord: prevChord.userData?.roman || 'I',
                                        progressionIndex: index,
                                        isProgression: true
                                    };
                ...
                if (lockedBass && bassEnabled) {
                    try {
                        ...
                        if (bassMidi) {
                            ...
                            const currentChord = lineup[index % lineup.length]?.userData?.roman || 'I';
                            const prevChord = index > 0 ? lineup[(index - 1) % lineup.length]?.userData?.roman || 'I' : null;
                            const context = {
                                currentChord,
                            ...
                            // HARD VOICE RANGE LIMITS: Apply constraints to prevent extreme ranges
                            bassMidi = constrainToVoiceRange(bassMidi, 'bass');

                            Tone.Draw.schedule(() => {
                                try {
                                    const bassNote = Tone.Frequency(bassMidi, "midi").toNote();
                                    console.log('[PROGRESSION] 🎵 Playing LOCKED bass with range limits:', bassNote, 'index:', index);
                                    window.audioEngine.playBass(bassNote, chordDurationSeconds, 0.8);
                                    lastBassMidi = bassMidi;
                                } catch (bassErr) {
                                    console.error('[PROGRESSION] Error playing locked bass:', bassErr);
                                }
                            }, time);
                        }
                    } catch (lockBassErr) {
                        console.error('[PROGRESSION] Error processing locked bass:', lockBassErr);
                    }
                }
                    ...
                    try {
                        ...
                        if (melMidi) {
                            ...
                            Tone.Draw.schedule(() => {
                                try {
                                    const melodyNote = Tone.Frequency(melMidi, "midi").toNote();
                                    console.log('[PROGRESSION] 🎵 Playing LOCKED melody with range limits:', melodyNote, 'index:', index);
                                    window.audioEngine.playMelody([melodyNote], [chordDurationSeconds], 0.5);
                                    lastMelodyMidi = melMidi;
                                } catch (melErr) {
                                    console.error('[PROGRESSION] Error playing locked melody:', melErr);
                                }
                            }, time);
                        }
                    } catch (lockMelErr) {
                        console.error('[PROGRESSION] Error processing locked melody:', lockMelErr);
                    }
                }
            } catch (seqErr) {
                console.error('[TRANSPORT] Error in sequence callback:', seqErr);
            }
        ...
        try {
            // Make sure Transport is started and running
            if (Tone.Transport.state !== "started") {
                console.log("[TRANSPORT] Starting Tone.js Transport - current state:", Tone.Transport.state);
                Tone.Transport.start();
            }

            // Configure transport with appropriate settings
            Tone.Transport.bpm.value = window.drumMachine?.bpm || 120;
            console.log("[TRANSPORT] Setting BPM to:", Tone.Transport.bpm.value);
            ...
            if (window.musicalStaves3D) {
                window.musicalStaves3D.setPlaybackActive(true);
                window.musicalStaves3D.displayChordProgression(lineup.map(c => ({ roman: c.userData.roman })));
                console.log('[TRANSPORT] 🎼 Staves notified of playback start');
            }

            // Start a safety timer to verify execution
            setTimeout(() => {
                if (window.chordProgressionSequence && progressionSequence.state === "stopped") {
                    console.warn("[TRANSPORT] Safety check - sequence appears to be stopped. Attempting to restart...");
                    try {
                        progressionSequence.start(0);
                    } catch (err) {
                        console.error("[TRANSPORT] Failed to restart sequence:", err);

                        // If restart fails, try the direct play approach
                        console.log("[TRANSPORT] Falling back to direct play approach...");
                        window.directPlayEnabled = true;
                        directPlayProgression().catch(directErr => {
                            console.error("[TRANSPORT] Direct play fallback also failed:", directErr);
                        });
                    }
                }
            }, 500);
            ...
            console.error("[TRANSPORT] Error starting progression sequence:", err);
            throw err; // Re-throw to trigger fallback
        }
    } catch (sequenceErr) {
        console.error("[TRANSPORT] Error creating or starting sequence:", sequenceErr);

        // Fall back to direct play method if sequence approach fails
        console.log("[TRANSPORT] Sequence-based play failed. Falling back to direct play approach");
        window.directPlayEnabled = true;
        return directPlayProgression();
    }
}
...
// Enhanced debug helper for audio system
window.debugAudio = function () {
    console.log('======= 🔊 AUDIO SYSTEM DIAGNOSTIC REPORT 🔊 =======');

    // Check Tone.js
    if (window.Tone) {
        console.log('Tone.js Status:');
        console.log('- Context State:', window.Tone.context?.state || 'No context');
        console.log('- Transport State:', window.Tone.Transport?.state || 'No transport');
        console.log('- Current BPM:', window.Tone.Transport?.bpm?.value || 'Unknown');
        console.log('- Destination Connected:', window.Tone.getDestination()?.numberOfInputs > 0 ? 'Yes' : 'No');

        // Try to check audio context connections
        try {
            const dest = window.Tone.getDestination();
            console.log('- Output Channels:', dest.channelCount);
            console.log('- Context Sample Rate:', window.Tone.context?.sampleRate || 'Unknown');
        } catch (err) {
            console.warn('Error checking Tone.js details:', err);
        }
    } else {
        console.warn('Tone.js not found in window object');
    }

    // Check AudioContextManager
    if (window.chordCubesTransport) {
        console.log('Transport Bridge Status:');
        console.log('- Has Manager:', !!window.chordCubesTransport);
        console.log('- Context State:', window.chordCubesTransport.getContextState?.() || 'Unknown');
    } else {
        console.warn('Transport bridge not found');
    }

    // Check Audio Engine
    if (window.audioEngine) {
        console.log('Audio Engine Status:');
        console.log('- Available:', !!window.audioEngine);
        console.log('- Has init method:', !!window.audioEngine.init);
        console.log('- Has playChord method:', !!window.audioEngine.playChord);
        console.log('- Has playBass method:', !!window.audioEngine.playBass);
        console.log('- Has playMelody method:', !!window.audioEngine.playMelody);
    } else {
    ...
    console.log('- WebAudio API available:', !!(window.AudioContext || window.webkitAudioContext));

    // Check if audio is allowed (only test if we have user gesture)
    try {
        if (document.hasStoredUserActivation !== false) {
            ...
            console.log('- Test context: Skipped (no user gesture yet)');
        }
    } catch (err) {
        console.log('- Test context: Failed to create');
    }

    console.log('======= END AUDIO DIAGNOSTIC REPORT =======');

    return "Audio diagnostic complete. Check console for details.";
};

// Force audio unlock with a user gesture
window.unlockAudio = function () {
    console.log('🔓 Attempting to unlock audio with user gesture...');

    // Method 1: Tone.js start
    if (window.Tone) {
        try {
            window.Tone.start();
            console.log('Tone.js start() called');
        } catch (err) {
            console.warn('Tone.js start failed:', err);
        }
    }

    // Method 2: Tone.context resume
    if (window.Tone && window.Tone.context) {
        try {
            window.Tone.context.resume();
            console.log('Tone.js context resumed');
        } catch (err) {
            console.warn('Tone.js context resume failed:', err);
        }
    }

    // Method 3: Direct WebAudio silent sound  
    try {
        // Only attempt if we have user gesture (prevents spam)
        if (document.hasStoredUserActivation !== false) {
            const unlockContext = new (window.AudioContext || window.webkitAudioContext)();
            const unlockOsc = unlockContext.createOscillator();
            const unlockGain = unlockContext.createGain();
            ...
            unlockOsc.start(0);
            unlockOsc.stop(unlockContext.currentTime + 0.001);
            console.log('Direct WebAudio unlock executed');
        }
    } catch (unlockErr) {
        console.warn('Direct WebAudio unlock failed:', unlockErr);
    }

    // Method 4: Transport bridge
    if (window.chordCubesTransport) {
        try {
            window.chordCubesTransport.ensureAudioContext();
            console.log('Transport bridge ensureAudioContext called');
        } catch (err) {
            console.warn('Transport bridge audio init failed:', err);
        }
    }

    // Method 5: Audio engine init
    if (window.audioEngine && !window.audioEngine.isReady && window.audioEngine.init) {
        try {
            window.audioEngine.init();
            console.log('Audio engine init called');
        } catch (err) {
            console.warn('Audio engine init failed:', err);
        }
    } else if (window.audioEngine && window.audioEngine.isReady) {
        console.log('Audio engine already ready - skipping debug init');
    }

    console.log('🔓 Audio unlock attempts complete');

    // Run diagnostics after unlock attempts
    setTimeout(() => window.debugAudio(), 500);

    return "Audio unlock attempts complete. Check console for details.";
}
...
// Font Ready Event Listener - triggers cube face refresh when font loads
document.addEventListener('fontReady', (e) => {
    console.log(`[FONT SYSTEM] ✅ Font loaded: ${e.detail.fontFamily}`);
    if (e.detail.fontFamily === 'Font Jan16' && typeof refreshAllCubeFaces === 'function') {
        console.log('[FONT SYSTEM] 🔄 Refreshing all cube faces with Font Jan16');
        refreshAllCubeFaces();
    }
});
...
// CRITICAL: Wait for fonts to load before initializing system
async function waitForFontsAndInitialize() {
    console.log('[FONT INIT] Waiting for Font Jan16 to load...');

    // Wait up to 3 seconds for fonts to load
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            // Test if Font Jan16 is available
            ctx.font = "400 100px FontJan16";
            const testWidth1 = ctx.measureText("I").width;
            ctx.font = "400 100px serif";
            const testWidth2 = ctx.measureText("I").width;

            if (Math.abs(testWidth1 - testWidth2) > 1) {
                console.log('[FONT INIT] ✅ Font Jan16 loaded successfully!');
                break;
            }
        } catch (e) {
            console.warn('[FONT INIT] Font test attempt failed:', e);
        }
    ...
    if (attempts >= maxAttempts) {
        console.error('[FONT INIT] ❌ Font Jan16 failed to load - using fallbacks');
    }

    // Initialize debug widget after font check
    if (document.getElementById('debug-cube-face')) {
        forensicDebugWidget = new ForensicDebugWidget();
        console.log('[🔬 FORENSIC DEBUG] ✅ Debug widget initialized after font load');
    } else {
        console.log('[🔬 FORENSIC DEBUG] Debug elements not found - real system will work unimpeded');
    }
}

// Initialize after DOM load with font waiting
// Add audio debug button setup to listen for audio init failures
function setupAudioDebugButton() {
    const audioDebugBtn = document.getElementById('audio-debug-btn');
    if (!audioDebugBtn) return;

    // Initial check if we need to show the button based on global flag
    // This gets set during audio initialization
    setTimeout(() => {
        if (window.audioInitialized === false) {
            console.log('[AUDIO DEBUG] Audio initialization failed initially, showing debug button');
            audioDebugBtn.style.display = 'block';
        }
    }, 2000); // Check after 2 seconds to give audio time to initialize

    // Setup click handler
    audioDebugBtn.addEventListener('click', async () => {
        console.log('[AUDIO DEBUG] Audio fix button clicked');

        // Visual feedback
        audioDebugBtn.innerText = '🔊 Working...';
        audioDebugBtn.style.background = '#009900';

        // Try to unlock audio
        if (window.unlockAudio) {
            await window.unlockAudio();
        } else {
            // Fallback if the helper function isn't defined
            try {
                if (window.Tone) {
                    await window.Tone.start();
                    console.log('[AUDIO DEBUG] Tone.js started');
                }

                if (window.chordCubesTransport) {
                    await window.chordCubesTransport.ensureAudioContext();
                    console.log('[AUDIO DEBUG] Transport ensureAudioContext called');
                }

                // Create silent oscillator hack (only with user gesture)
                if (document.hasStoredUserActivation !== false) {
                    ...
                    console.log('[AUDIO DEBUG] Silent oscillator created');
                }
            } catch (err) {
                console.error('[AUDIO DEBUG] Error unlocking audio:', err);
            }
        }

        // Check if it worked
        setTimeout(() => {
            const contextRunning = window.Tone?.context?.state === 'running';
            const transportRunning = window.chordCubesTransport?.getContextState?.() === 'running';

            if (contextRunning || transportRunning) {
                audioDebugBtn.innerText = '✅ Fixed!';
                window.audioInitialized = true;

                // Hide the button after a short delay
                setTimeout(() => {
                    audioDebugBtn.style.display = 'none';
                }, 3000);
            } else {
                audioDebugBtn.innerText = '❌ Try Again';
                audioDebugBtn.style.background = '#cc0000';
            }
        }, 1000);
    });

    console.log('[AUDIO DEBUG] Audio debug button setup complete');
}

document.addEventListener('DOMContentLoaded', waitForFontsAndInitialize);
// Add audio debug button setup
document.addEventListener('DOMContentLoaded', setupAudioDebugButton);
...
if (document.readyState === 'loading') {
    ...
    // Also setup audio debug button
    setupAudioDebugButton();
}
